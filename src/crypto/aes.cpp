...
bool aes::remove_pkcs7_padding(std::vector<uint8_t> &data, uint32_t real_size) {
    do {
...
bool aes::pre_process(const std::vector<uint8_t> &key_bytes,
    ...
    do {
        ...
        if (!CryptImportKey(crypt_prov_handle,
                reinterpret_cast<uint8_t *>(&blob), sizeof(blob), 0, 0,
                &crypt_key_handle)) {
            error_code = GetLastError();
            break;
        }
...
bool aes::encode_process(std::vector<uint8_t> &data_bytes, AesPadding padding) {
    do {
        switch (padding) {
...
bool aes::decode_process(std::vector<uint8_t> &data_bytes, AesPadding padding) {
    do {
        uint32_t data_size = data_bytes.size();
...
std::vector<uint8_t> aes::encode_cbc(const std::vector<uint8_t> &data,
    ...
    do {
        ...
        if (!pre_process(key_bytes, localIV, AesMode::CBC)) {
            break;
        }
