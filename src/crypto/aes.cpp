...
namespace YanLib::crypto {
    ...
    std::string aes::format_hex_fast(const std::vector<unsigned char> &data) {
    ...
    aes::KeyBlob aes::make_blob(const std::vector<unsigned char> &key) {
        KeyBlob blob{};
        blob.header = {
            PLAINTEXTKEYBLOB,
            CUR_BLOB_VERSION,
    ...
    void aes::make_pkcs7_padding(std::vector<unsigned char> &data) {
    ...
    bool aes::remove_pkcs7_padding(std::vector<unsigned char> &data,
                                   DWORD retLen) {
        do {
    ...
    void aes::make_iso10126_padding(std::vector<unsigned char> &data) {
        const size_t rawSize = data.size();
        const size_t padLen = 16 - (rawSize % 16);
        if (padLen == 0) {
            return;
    ...
    bool aes::remove_iso10126_padding(std::vector<unsigned char> &data,
                                      DWORD retLen) {
        do {
            if (data.empty() || retLen < 1 || retLen > data.size()) {
                break;
    ...
    void aes::make_ansix923_padding(std::vector<unsigned char> &data) {
        const size_t rawSize = data.size();
        const size_t padLen = 16 - (rawSize % 16);
        if (padLen == 0) {
            return;
    ...
    bool aes::remove_ansix923_padding(std::vector<unsigned char> &data,
                                      DWORD retLen) {
        do {
            if (data.empty() || retLen < 1 || retLen > data.size()) {
                break;
    ...
    bool aes::pre_process(const std::vector<unsigned char> &key_bytes,
    ...
    bool aes::encode_process(std::vector<unsigned char> &data_bytes,
                             AES_PADDING padding) {
        do {
            switch (padding) {
                case PKCS7_PADDING:
    ...
    bool aes::decode_process(std::vector<unsigned char> &data_bytes,
                             AES_PADDING padding) {
        do {
            DWORD dataSize = data_bytes.size();
    ...
    std::vector<unsigned char> aes::encode_cbc(const std::vector<unsigned char> &data,
    ...
    std::vector<unsigned char> aes::decode_cbc(const std::vector<unsigned char> &data,
                                               const std::vector<unsigned char> &key,
                                               const std::vector<unsigned char> &iv,
                                               AES_PADDING padding) {
        if (data.empty() ||
    ...
    std::vector<unsigned char> aes::encode_ecb(const std::vector<unsigned char> &data,
                                               const std::vector<unsigned char> &key,
                                               AES_PADDING padding) {
        if (data.empty() ||
            key.empty() ||
    ...
    std::vector<unsigned char> aes::decode_ecb(const std::vector<unsigned char> &data,
                                               const std::vector<unsigned char> &key,
                                               AES_PADDING padding) {
        if (data.empty() ||
            key.empty() ||
    ...
    std::vector<unsigned char> aes::encode_cfb(const std::vector<unsigned char> &data,
                                               const std::vector<unsigned char> &key,
                                               const std::vector<unsigned char> &iv,
                                               AES_PADDING padding) {
        if (data.empty() ||
    ...
    std::vector<unsigned char> aes::decode_cfb(const std::vector<unsigned char> &data,
                                               const std::vector<unsigned char> &key,
                                               const std::vector<unsigned char> &iv,
                                               AES_PADDING padding) {
        if (data.empty() ||
    ...
    std::vector<unsigned char> aes::generate_iv_bytes() {
        do {
            wchar_t provider[] =
                    L"Microsoft Enhanced RSA and AES Cryptographic Provider";
            if (!CryptAcquireContextW(&hCryptProv,
