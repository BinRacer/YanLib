...
namespace YanLib::crypto {
    std::vector<unsigned char> base16::encode(const std::vector<unsigned char> &data) {
        constexpr unsigned char BASE16_CHARS[] =
                "0123456789ABCDEF";
    ...
    std::vector<unsigned char> base16::decode(const std::vector<unsigned char> &data) {
        if (data.empty() || data.size() % 2 != 0) return {};

        auto char_to_nibble = [](const unsigned char c) -> int {
