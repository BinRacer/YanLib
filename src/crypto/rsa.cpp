...
bool rsa::generate_key(RsaKeyBits key_bits) {
    do {
        ...
        if (!CryptAcquireContextW(
                &crypt_prov_handle, nullptr, provider, PROV_RSA_AES, 0)) {
        ...
        if (!CryptExportKey(crypt_key_handle, 0, PUBLICKEYBLOB, 0,
                pub_key.data(), reinterpret_cast<unsigned long *>(&publen))) {
            error_code = GetLastError();
            break;
        }
        uint32_t privlen = 0;
        ...
        if (!CryptExportKey(crypt_key_handle, 0, PRIVATEKEYBLOB, 0,
                priv_key.data(), reinterpret_cast<unsigned long *>(&privlen))) {
            error_code = GetLastError();
            break;
        }
        cleanup();
...
std::vector<uint8_t> rsa::encode(const std::vector<uint8_t> &data,
    ...
    do {
        ...
        if (!CryptImportKey(crypt_prov_handle, pub_blob.data(), pub_blob.size(),
