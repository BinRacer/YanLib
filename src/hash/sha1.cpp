...
namespace YanLib::hash {
    sha1::sha1(const std::vector<unsigned char> &data) {
        hCryptProv = 0;
        hCryptHash = 0;
        data_bytes = data;
        hash_bytes = {};
    ...
    std::string sha1::format_hex_fast(const std::vector<unsigned char> &data) {
    ...
    std::vector<unsigned char> sha1::process() {
        do {
            if (!pre_process()) {
                break;
            }
    ...
    std::vector<unsigned char> sha1::hash() {
        return process();
    }
