...
namespace YanLib::hash {
    ...
    sha1::sha1(const char *filename, helper::CodePage code_page) {
        crypt_prov_handle = 0;
        crypt_hash_handle = 0;
        data_bytes = {};
        hash_bytes = {};
    ...
    bool sha1::pre_process() {
        ...
        do {
            ...
            if (!CryptCreateHash(crypt_prov_handle, CALG_SHA1, 0, 0,
                                 &crypt_hash_handle)) {
                error_code = GetLastError();
                break;
            }
    ...
    std::wstring sha1::hash_wstring(helper::CodePage code_page) {
        return helper::convert::str_to_wstr(hash_string(), code_page);
    }

    uint32_t sha1::err_code() const {
        return error_code;
    }
