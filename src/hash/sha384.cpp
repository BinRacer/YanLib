...
namespace YanLib::hash {
    sha384::sha384(const std::vector<unsigned char> &data) {
        hCryptProv = 0;
        hCryptHash = 0;
        data_bytes = data;
        hash_bytes = {};
    ...
    sha384::sha384(const std::wstring &filename) {
        hCryptProv = 0;
        hCryptHash = 0;
        data_bytes = {};
        hash_bytes = {};
    ...
    std::string sha384::format_hex_fast(const std::vector<unsigned char> &data) {
    ...
    std::vector<unsigned char> sha384::process() {
        do {
            if (!pre_process()) {
                break;
            }
    ...
    std::vector<unsigned char> sha384::hash() {
        if (isDone) {
            return hash_bytes;
        }
        return process();
