...
sha384::sha384(const char *filename) {
    crypt_prov_handle = 0;
    crypt_hash_handle = 0;
    data_bytes        = {};
    hash_bytes        = {};
...
sha384::sha384(const wchar_t *filename) {
    crypt_prov_handle = 0;
    crypt_hash_handle = 0;
    data_bytes        = {};
    hash_bytes        = {};
...
std::string sha384::format_hex_fast(const std::vector<uint8_t> &data) {
...
bool sha384::pre_process() {
    do {
        ...
        if (!CryptCreateHash(
                crypt_prov_handle, CALG_SHA_384, 0, 0, &crypt_hash_handle)) {
            error_code = GetLastError();
            break;
        }
        return true;
...
std::vector<uint8_t> sha384::process() {
    do {
        if (!pre_process()) {
            break;
        }
...
std::vector<uint8_t> sha384::hash() {
    if (is_done) {
        return hash_bytes;
    }
    return process();
...
std::wstring sha384::hash_wstring() {
    return helper::convert::str_to_wstr(hash_string());
}

uint32_t sha384::err_code() const {
    return error_code;
}

std::string sha384::err_string() const {
    std::string result = helper::convert::err_string(error_code);
    return result;
}

std::wstring sha384::err_wstring() const {
