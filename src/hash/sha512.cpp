...
sha512::sha512(const std::vector<uint8_t> &data) {
    crypt_prov_handle = 0;
    crypt_hash_handle = 0;
    data_bytes        = data;
    hash_bytes        = {};
...
sha512::sha512(const char *filename) {
    crypt_prov_handle = 0;
    crypt_hash_handle = 0;
    data_bytes        = {};
    hash_bytes        = {};
...
sha512::sha512(const wchar_t *filename) {
    crypt_prov_handle = 0;
    crypt_hash_handle = 0;
    data_bytes        = {};
    hash_bytes        = {};
...
std::string sha512::format_hex_fast(const std::vector<uint8_t> &data) {
...
bool sha512::pre_process() {
    do {
        ...
        if (!CryptCreateHash(
                crypt_prov_handle, CALG_SHA_512, 0, 0, &crypt_hash_handle)) {
            error_code = GetLastError();
            break;
        }
        return true;
...
bool sha512::process_file() {
    do {
        ...
        if (file.size() <= 4096) {
            ...
            if (!CryptHashData(
                    crypt_hash_handle, data.data(), data.size(), 0)) {
                error_code = GetLastError();
                break;
            }
        } else {
...
std::vector<uint8_t> sha512::process() {
    do {
        if (!pre_process()) {
            break;
        }
...
std::vector<uint8_t> sha512::hash() {
    if (is_done) {
        return hash_bytes;
    }
    return process();
...
std::wstring sha512::hash_wstring() {
    return helper::convert::str_to_wstr(hash_string());
}

uint32_t sha512::err_code() const {
    return error_code;
}

std::string sha512::err_string() const {
    std::string result = helper::convert::err_string(error_code);
    return result;
}

std::wstring sha512::err_wstring() const {
