...
namespace YanLib::hash {
    sha512::sha512(const std::vector<unsigned char> &data) {
        hCryptProv = 0;
        hCryptHash = 0;
        data_bytes = data;
        hash_bytes = {};
    ...
    sha512::sha512(const std::wstring &filename) {
        hCryptProv = 0;
        hCryptHash = 0;
        data_bytes = {};
        hash_bytes = {};
    ...
    std::string sha512::format_hex_fast(const std::vector<unsigned char> &data) {
    ...
    std::vector<unsigned char> sha512::process() {
        do {
            if (!pre_process()) {
                break;
            }
    ...
    std::vector<unsigned char> sha512::hash() {
        return process();
    }
