...
bool comp_port::create() {
    iocp = CreateIoCompletionPort(
        INVALID_HANDLE_VALUE, nullptr, 0, max_concurrent_threads);
    if (!iocp) {
...
bool comp_port::post_status(ULONG_PTR comp_key,
    ...
    if (!PostQueuedCompletionStatus(iocp, num_bytes, comp_key, overlapped)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool comp_port::get_status(ULONG_PTR *comp_key,
    ...
    if (!GetQueuedCompletionStatus(iocp,
            reinterpret_cast<unsigned long *>(num_bytes), comp_key, overlapped,
            milli_seconds)) {
        error_code = GetLastError();
        return false;
    }
...
bool comp_port::get_status(OVERLAPPED_ENTRY *comp_port_entries,
    ...
    if (!GetQueuedCompletionStatusEx(iocp, comp_port_entries, count,
            reinterpret_cast<unsigned long *>(num_entries_removed),
            milli_seconds, is_alert ? TRUE : FALSE)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool comp_port::device_io_control(HANDLE device_handle,
    ...
    if (!DeviceIoControl(device_handle, io_control_code, in_buffer,
            ...
            reinterpret_cast<unsigned long *>(bytes_transferred), overlapped)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool comp_port::device_io_control(HANDLE device_handle,
    ...
    if (!DeviceIoControl(device_handle, io_control_code, in_buffer.data(),
            in_buffer.size(), out_buffer.data(), out_buffer.size(),
            reinterpret_cast<unsigned long *>(bytes_transferred), overlapped)) {
        error_code = GetLastError();
        return false;
    }
...
bool comp_port::get_overlapped_result(HANDLE file_handle,
    ...
    if (!GetOverlappedResult(file_handle, overlapped,
            reinterpret_cast<unsigned long *>(bytes_transferred),
            is_wait ? TRUE : FALSE)) {
        error_code = GetLastError();
        return false;
    }
...
bool comp_port::get_overlapped_result(HANDLE file_handle,
    ...
    if (!GetOverlappedResultEx(file_handle, overlapped,
            reinterpret_cast<unsigned long *>(bytes_transferred), milli_seconds,
            is_wait ? TRUE : FALSE)) {
        error_code = GetLastError();
        return false;
    }
