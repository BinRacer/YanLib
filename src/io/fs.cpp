...
void fs::remove_tail_slash(std::string &path) {
    std::transform(path.begin(), path.end(), path.begin(), [](const char ch) {
        return (ch == '/') ? '\\' : ch;
    });
    if (path.back() == '\\') {
        path.pop_back();
...
void fs::remove_tail_slash(std::wstring &path) {
    std::transform(
        path.begin(), path.end(), path.begin(), [](const wchar_t ch) {
            return (ch == L'/') ? L'\\' : ch;
        });
    if (path.back() == L'\\') {
        path.pop_back();
...
fs::fs(const char       *file_name,
    ...
    file_handle = CreateFileA(file_name, static_cast<uint32_t>(desired_access),
        static_cast<uint32_t>(share_mode), security_attrs,
        static_cast<uint32_t>(creation_disposition),
        static_cast<uint32_t>(flag_and_attr), template_file);
...
fs::fs(const wchar_t    *file_name,
    ...
    FlagAndAttr          flag_and_attr,
    HANDLE               template_file) {
    file_handle = CreateFileW(file_name, static_cast<uint32_t>(desired_access),
        static_cast<uint32_t>(share_mode), security_attrs,
        static_cast<uint32_t>(creation_disposition),
        static_cast<uint32_t>(flag_and_attr), template_file);
    if (file_handle == INVALID_HANDLE_VALUE) {
...
bool fs::read(void *buf,
    ...
    if (!ReadFile(file_handle, buf, size,
            reinterpret_cast<unsigned long *>(bytes_read), overlapped)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool fs::read(void               *buf,
    ...
    if (!ReadFileEx(file_handle, buf, size, overlapped, completion_routine)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool fs::write(const void *buf,
    ...
    OVERLAPPED            *overlapped) {
    if (!WriteFile(file_handle, buf, size,
            reinterpret_cast<unsigned long *>(bytes_written), overlapped)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool fs::write(const void        *buf,
    ...
    OVERLAPPED                   *overlapped,
    OVERLAPPED_COMPLETION_ROUTINE completion_routine) {
    if (!WriteFileEx(file_handle, buf, size, overlapped, completion_routine)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
std::vector<uint8_t> fs::read_bytes(int32_t buffer_size) {
    ...
    if (ReadFile(file_handle, raw_data.data(), buffer_size,
            reinterpret_cast<unsigned long *>(&bytes_read), nullptr) &&
        bytes_read > 0) {
...
std::vector<uint8_t> fs::read_bytes_to_end() {
    ...
    uint32_t bytes_read = 0;
    do {
        if (ReadFile(file_handle, buf.data(), buf.size(),
                reinterpret_cast<unsigned long *>(&bytes_read), nullptr) &&
            bytes_read > 0) {
            raw_data.insert(
                raw_data.end(), buf.data(), buf.data() + bytes_read);
        } else {
            error_code = GetLastError();
            break;
        }
...
uint32_t fs::write_bytes(const std::vector<uint8_t> &data) {
    ...
    if (!WriteFile(file_handle, data.data(), data.size(),
            reinterpret_cast<unsigned long *>(&bytes_written), nullptr)) {
        error_code = GetLastError();
    }
...
int64_t fs::seek(int64_t offset, MoveMethod move_method) {
    ...
    if (!SetFilePointerEx(
            file_handle, move, &new_pos, static_cast<uint32_t>(move_method))) {
        error_code = GetLastError();
        return 0;
    }
...
bool fs::lock(uint64_t offset, uint64_t range) {
    ...
    if (!LockFile(
            file_handle, offset_low, offset_high, range_low, range_high)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool fs::lock_async(LockMode lock_mode,
    ...
    if (!LockFileEx(file_handle, static_cast<uint32_t>(lock_mode), 0, low, high,
            overlapped)) {
        error_code = GetLastError();
        return false;
    }
...
bool fs::get_info(FILE_INFO_BY_HANDLE_CLASS file_info_class,
    ...
    if (!GetFileInformationByHandleEx(
            file_handle, file_info_class, file_info, file_info_size)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool fs::get_volume_info(VolumeInfoA *volume_info) {
    ...
    if (get_volume_info(volume_info_wide.get())) {
        ...
        auto file_system_name =
            helper::convert::wstr_to_str(volume_info_wide->file_system_name);
...
bool fs::get_volume_info(VolumeInfoW *volume_info) {
    ...
    uint32_t size = 0;
    if (!GetVolumeInformationByHandleW(file_handle, volume_info->volume_name,
            MAX_PATH,
            reinterpret_cast<unsigned long *>(&(volume_info->serial_number)),
            reinterpret_cast<unsigned long *>(&size),
            reinterpret_cast<unsigned long *>(&(volume_info->file_system_flag)),
            volume_info->file_system_name, MAX_PATH)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool fs::get_final_path_name(std::wstring &path_name, PathNameType type) {
    uint32_t size = GetFinalPathNameByHandleW(
        file_handle, nullptr, 0, static_cast<uint32_t>(type));
    path_name.resize(size, L'\0');
    size = GetFinalPathNameByHandleW(file_handle, path_name.data(),
        path_name.size(), static_cast<uint32_t>(type));
    if (!size) {
        error_code = GetLastError();
        return false;
    }
...
bool fs::monitor_dir_change(void *buffer,
    ...
    if (!ReadDirectoryChangesW(file_handle, buffer, buffer_length,
            is_watch_subtree ? TRUE : FALSE, notify_filter,
            reinterpret_cast<unsigned long *>(bytes_returned), overlapped,
            completion_routine)) {
        error_code = GetLastError();
        return false;
    }
...
std::string fs_util::get_volume_path_names_for_volume_name(
    const char *volume_name) {
    uint32_t size = 0;
    if (!GetVolumePathNamesForVolumeNameA(volume_name, nullptr, 0,
            reinterpret_cast<unsigned long *>(&size))) {
        if (GetLastError() == ERROR_MORE_DATA) {
            std::string buffer(size, '\0');
            if (!GetVolumePathNamesForVolumeNameA(volume_name, buffer.data(),
                    buffer.size(), reinterpret_cast<unsigned long *>(&size))) {
                error_code = GetLastError();
                return {};
            }
            buffer.resize(size);
...
std::wstring fs_util::get_volume_path_names_for_volume_name(
    const wchar_t *volume_name) {
    uint32_t size = 0;
    if (!GetVolumePathNamesForVolumeNameW(volume_name, nullptr, 0,
            reinterpret_cast<unsigned long *>(&size))) {
        if (GetLastError() == ERROR_MORE_DATA) {
...
std::string fs_util::get_volume_name_for_volume_mount_point(
    ...
    if (!GetVolumeNameForVolumeMountPointA(
            volume_mount_point, buffer.data(), MAX_PATH)) {
        error_code = GetLastError();
        return {};
    }
    while (buffer.back() == '\0') {
...
std::string fs_util::get_full_path_name(const char *file_name) {
    ...
    std::string buffer(size, '\0');
    size = GetFullPathNameA(file_name, buffer.size(), buffer.data(), nullptr);
    if (!size) {
        error_code = GetLastError();
        return {};
    }
...
std::wstring fs_util::get_full_path_name(const wchar_t *file_name) {
    ...
    std::wstring buffer(size, L'\0');
    size = GetFullPathNameW(file_name, buffer.size(), buffer.data(), nullptr);
    if (!size) {
        error_code = GetLastError();
        return {};
    }
...
bool fs_util::get_disk_free_space(const char *root_path_name,
    ...
    if (!GetDiskFreeSpaceA(root_path_name,
...
bool fs_util::get_disk_free_space(const char *directory_name,
    ...
    disk_free_space3->total_number_of_bytes = total_number_of_bytes.QuadPart;
...
int64_t fs_util::get_compressed_file_size(const char *file_name) {
    uint32_t high = 0;
    uint32_t low  = GetCompressedFileSizeA(
        file_name, reinterpret_cast<unsigned long *>(&high));
...
int64_t fs_util::get_compressed_file_size(const wchar_t *file_name) {
    uint32_t high = 0;
    uint32_t low  = GetCompressedFileSizeW(
        file_name, reinterpret_cast<unsigned long *>(&high));
    if (low == INVALID_FILE_SIZE) {
        error_code = GetLastError();
        if (error_code != NO_ERROR) {
...
bool fs_util::control_dos_device(const char *device_name,
    ...
    DosControlCode                           code) {
    if (!DefineDosDeviceA(
            static_cast<uint32_t>(code), device_name, target_path)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool fs_util::ls_volume_name(std::vector<std::string> &volume_names) {
    do {
        ...
        } while (FindNextVolumeA(
            file_handle, volume_name.data(), volume_name.size()));
...
bool fs_util::ls_device_name(std::vector<std::string> &device_names) {
    ...
    for (auto volume : volumes) {
        ...
        if (!QueryDosDeviceA(
                volume.data() + 4, device_name.data(), device_name.size())) {
            error_code = GetLastError();
            break;
        }
...
std::vector<WIN32_FIND_STREAM_DATA> fs_util::ls_stream_data(
...
std::vector<WIN32_FIND_STREAM_DATA> fs_util::ls_stream_data(
    ...
    do {
        ...
        HANDLE                              file_handle = FindFirstStreamW(
            file_name, FindStreamInfoStandard, &stream_data, 0);
        if (file_handle == INVALID_HANDLE_VALUE) {
            error_code = GetLastError();
            break;
        }
...
void fs_util::rm_dir_all_slow(const char *path_name) {
    ...
    std::for_each(
        files.rbegin(), files.rend(), [this](const std::string &file) {
            rm_file(file.data());
        });
    std::for_each(dirs.rbegin(), dirs.rend(), [this](const std::string &dir) {
        rm_dir(dir.data());
    });
}
...
void fs_util::rm_dir_all_slow(const wchar_t *path_name) {
    ...
    std::for_each(
        files.rbegin(), files.rend(), [this](const std::wstring &file) {
            rm_file(file.data());
        });
    std::for_each(dirs.rbegin(), dirs.rend(), [this](const std::wstring &dir) {
        rm_dir(dir.data());
    });
}
...
bool fs_util::copy(const char *existing_file_name,
    ...
    bool                       fail_if_exists) {
    if (!CopyFileA(
            existing_file_name, new_file_name, fail_if_exists ? TRUE : FALSE)) {
        error_code = GetLastError();
        return false;
    }
    return true;
