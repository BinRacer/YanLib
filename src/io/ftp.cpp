...
bool ftp::connect(uint32_t service, uint32_t flag, DWORD_PTR context) {
    session_read_handle = InternetConnectW(internet_handle, _hostname, _port,
        wcslen(_username) == 0 ? nullptr : _username,
        wcslen(_password) == 0 ? nullptr : _password, service, flag, context);
    if (!session_read_handle) {
        error_code = GetLastError();
        return false;
    }
    session_upload_handle = InternetConnectW(internet_handle, _hostname, _port,
        wcslen(_username) == 0 ? nullptr : _username,
        wcslen(_password) == 0 ? nullptr : _password, service, flag, context);
    if (!session_upload_handle) {
        error_code = GetLastError();
...
HINTERNET ftp::open_file(const char *file_name,
    ...
    uint32_t                         flag,
    DWORD_PTR                        context) {
    HINTERNET file_handle =
        FtpOpenFileA(session_read_handle, file_name, access, flag, context);
    if (!file_handle) {
        error_code = GetLastError();
        return nullptr;
    }
...
HINTERNET ftp::open_file(const wchar_t *file_name,
    ...
    DWORD_PTR                           context) {
    HINTERNET file_handle =
        FtpOpenFileW(session_read_handle, file_name, access, flag, context);
    if (!file_handle) {
        error_code = GetLastError();
        return nullptr;
    }
...
HINTERNET ftp::create_file(const char *file_name,
    ...
    DWORD_PTR                          context) {
    HINTERNET file_handle =
        FtpOpenFileA(session_upload_handle, file_name, access, flag, context);
    if (!file_handle) {
        error_code = GetLastError();
        return nullptr;
    }
...
HINTERNET ftp::create_file(const wchar_t *file_name,
    ...
    DWORD_PTR                             context) {
    HINTERNET file_handle =
        FtpOpenFileW(session_upload_handle, file_name, access, flag, context);
    if (!file_handle) {
        error_code = GetLastError();
        return nullptr;
    }
...
int64_t ftp::size(HINTERNET file_handle) {
    ...
    low = FtpGetFileSize(file_handle, reinterpret_cast<unsigned long *>(&high));
...
bool ftp::ls(std::vector<std::string> &files, std::vector<std::string> &dirs) {
    ...
    HINTERNET        find_handle = FtpFindFirstFileA(session_read_handle,
               current_dir.data(), &file_data, INTERNET_FLAG_RELOAD, 0);
    if (!find_handle) {
        error_code = GetLastError();
        return false;
    }
...
bool ftp::ls(std::vector<std::wstring> &files,
    ...
    HINTERNET        find_handle = FtpFindFirstFileW(session_read_handle,
               current_dir.data(), &file_data, INTERNET_FLAG_RELOAD, 0);
    if (!find_handle) {
        error_code = GetLastError();
        return false;
...
bool ftp::ls_full_path(std::vector<std::string> &files,
    ...
    HINTERNET        find_handle = FtpFindFirstFileA(session_read_handle,
               current_dir.data(), &find_file_data, INTERNET_FLAG_RELOAD, 0);
    if (!find_handle) {
        error_code = GetLastError();
        return false;
    }
...
bool ftp::download(const char *remote_file,
    ...
    if (!FtpGetFileA(session_read_handle, remote_file, new_file,
            is_fail_if_exists ? TRUE : FALSE, flags_and_attrs, flag, context)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool ftp::upload(const char *local_file,
    ...
    uint32_t                 flag,
    DWORD_PTR                context) {
    if (!FtpPutFileA(session_upload_handle, local_file, new_remote_file, flag,
            context)) {
        error_code = GetLastError();
        return false;
    }
...
bool ftp::upload(const wchar_t *local_file,
    ...
    uint32_t                    flag,
    DWORD_PTR                   context) {
    if (!FtpPutFileW(session_upload_handle, local_file, new_remote_file, flag,
            context)) {
        error_code = GetLastError();
        return false;
    }
