...
public:
    ...
    HINTERNET open_file(const char *file_name,
        ...
        uint32_t flag = FTP_TRANSFER_TYPE_BINARY | INTERNET_FLAG_RELOAD |
    ...
    bool
    read(HINTERNET file_handle, void *buf, uint32_t size, uint32_t *ret_size);
    ...
    bool ls(std::vector<std::string> &files, std::vector<std::string> &dirs);

    bool ls(std::vector<std::wstring> &files, std::vector<std::wstring> &dirs);
