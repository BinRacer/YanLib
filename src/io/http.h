...
public:
    ...
    bool get_headers(std::unordered_map<std::wstring, std::wstring> &headers);

    bool get_headers_vec(std::vector<std::string> &headers);

    bool get_headers_vec(std::vector<std::wstring> &headers);

    bool query_option(uint32_t option, void *buffer, uint32_t *buffer_length);
    ...
    bool open_request(const char *verb, /* "GET" */
    ...
    static std::vector<uint8_t> read_bytes_to_end(const std::string &input_url);
    ...
    static uint32_t upload(const std::string &input_url, const char *file_name);
