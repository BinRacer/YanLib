...
namespace YanLib::io {
    ...
    bool pe32::get_nt_signature(uint32_t &signature) {
        if (!mmap.read(_nt_headers, reinterpret_cast<uint8_t*>(&signature),
                       sizeof(uint32_t),
                       offsetof(IMAGE_NT_HEADERS32, Signature))) {
            error_code = mmap.err_code();
            return false;
        }
        return true;
    ...
    std::vector<IMAGE_SECTION_HEADER> pe32::get_section_headers() {
        ...
        if (!mmap.read(_section_header,
                       ...
                       static_cast<int64_t>(sizeof(IMAGE_SECTION_HEADER) *
                                            count))) {
            error_code = mmap.err_code();
            return {};
        }
    ...
    pe32::get_export_func_name_string(IMAGE_EXPORT_DIRECTORY* export_table) {
        if (!export_table) {
            return {};
        }
    ...
    std::vector<uint32_t>
    pe32::get_export_func_name(IMAGE_EXPORT_DIRECTORY* export_table) {
        if (!export_table) {
            return {};
        }
    ...
    std::vector<uint32_t>
    pe32::get_export_func_addr(IMAGE_EXPORT_DIRECTORY* export_table) {
        if (!export_table) {
            return {};
        }
    ...
    std::vector<uint16_t>
    pe32::get_export_func_ordinal(IMAGE_EXPORT_DIRECTORY* export_table) {
        if (!export_table) {
            return {};
        }
    ...
    std::vector<IMAGE_IMPORT_DESCRIPTOR> pe32::get_import_table() {
        ...
        while (import_table->OriginalFirstThunk ||
               import_table->TimeDateStamp || import_table->ForwarderChain ||
               import_table->Name || import_table->FirstThunk) {
            count++;
            import_table++;
        }
        std::vector<IMAGE_IMPORT_DESCRIPTOR> result(count, {});
        if (!mmap.read(_dos_header, reinterpret_cast<uint8_t*>(result.data()),
                       static_cast<int64_t>(sizeof(IMAGE_IMPORT_DESCRIPTOR) *
                                            count),
                       offset)) {
            error_code = mmap.err_code();
            return {};
    ...
    bool
    pe32::set_import_table(std::vector<IMAGE_IMPORT_DESCRIPTOR> &import_table) {
    ...
    pe32::get_import_table_thunk_data(uint32_t &first_thunk) {
        ...
        if (!mmap.read(_dos_header, reinterpret_cast<uint8_t*>(result.data()),
                       static_cast<int64_t>(sizeof(IMAGE_THUNK_DATA32) * count),
                       offset)) {
            error_code = mmap.err_code();
            return {};
        }
    ...
    bool pe32::set_import_table_thunk_data(
        ...
        if (!mmap.write(_dos_header,
                        ...
                        static_cast<int64_t>(sizeof(IMAGE_THUNK_DATA32) *
                                             thunk_datas.size()),
                        offset)) {
            error_code = mmap.err_code();
            return false;
    ...
    pe32::get_import_table_func_ordinal(
            std::vector<IMAGE_THUNK_DATA32> &thunk_datas) {
        if (thunk_datas.empty()) {
            return {};
        }
    ...
    bool pe32::set_import_table_func_ordinal(
            uint32_t &first_thunk,
            std::vector<IMAGE_THUNK_DATA32> &thunk_datas,
    ...
    bool
    pe32::set_relocation_table(std::vector<RelocationTable> &relocation_table) {
        if (_data_table_list.empty()) {
            get_data_table();
        }
        if (_data_table_list[DIRECTORY_ENTRY_BASE_RELOC].Size == 0) {
        ...
        for (const auto &entry : relocation_table) {
            ...
            memcpy_s(item_addr, sizeof(uint16_t) * item_count,
                     entry.items.data(), sizeof(uint16_t) * item_count);
    ...
    pe32::get_relocation_table_item(RelocationTable &relocation) {
        ...
        for (const auto &entry : relocation.items) {
            ...
            result.emplace_back(entry, type,
                                (IMAGE_REL_BASED_HIGHLOW == type) ? real_addr
                                                                  : 0);
        }
        return result;
    }

    bool pe32::set_relocation_table_item(
            RelocationTable &relocation,
    ...
    uint32_t pe32::foa_to_rva(IMAGE_SECTION_HEADER &section_header,
                              int64_t foa) {
