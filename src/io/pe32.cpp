...
namespace YanLib::io {
pe32::pe32(const char *file_name) {
    do {
...
pe32::pe32(const wchar_t *file_name) {
    do {
        mmap_handle = mmap.create(file_name, nullptr);
        if (!mmap_handle) {
            error_code = mmap.err_code();
            break;
        }
...
pe32::pe32(HANDLE file_handle) {
    do {
        mmap_handle = mmap.create(file_handle, static_cast<char *>(nullptr));
        if (!mmap_handle) {
            error_code = mmap.err_code();
            break;
        }
...
void pe32::init() {
    do {
        ...
        _nt_headers = reinterpret_cast<IMAGE_NT_HEADERS32 *>(
            static_cast<uint8_t *>(addr) + _dos_header->e_lfanew);
...
bool pe32::parse() const {
    if (!mmap_handle || !addr || !_dos_header || !_nt_headers ||
        !_section_header) {
        return false;
    }
    return true;
...
bool pe32::is_32bits() const {
    return !is_64bit;
}

bool pe32::is_64bits() const {
    return is_64bit;
}

bool pe32::get_dos_header(IMAGE_DOS_HEADER &dos_header) {
    if (!mmap.read(_dos_header, reinterpret_cast<uint8_t *>(&dos_header),
...
bool pe32::set_dos_header(IMAGE_DOS_HEADER &dos_header) {
    if (!mmap.write(_dos_header, reinterpret_cast<uint8_t *>(&dos_header),
            sizeof(IMAGE_DOS_HEADER))) {
        error_code = mmap.err_code();
        return false;
    }
...
bool pe32::get_nt_headers(IMAGE_NT_HEADERS32 &nt_headers) {
    if (!mmap.read(_nt_headers, reinterpret_cast<uint8_t *>(&nt_headers),
            sizeof(IMAGE_NT_HEADERS32))) {
        error_code = mmap.err_code();
        return false;
    }
...
bool pe32::set_nt_headers(IMAGE_NT_HEADERS32 &nt_headers) {
    if (!mmap.write(_nt_headers, reinterpret_cast<uint8_t *>(&nt_headers),
            sizeof(IMAGE_NT_HEADERS32))) {
        error_code = mmap.err_code();
        return false;
    }
...
bool pe32::get_nt_signature(uint32_t &signature) {
    if (!mmap.read(_nt_headers, reinterpret_cast<uint8_t *>(&signature),
            sizeof(uint32_t), offsetof(IMAGE_NT_HEADERS32, Signature))) {
        error_code = mmap.err_code();
        return false;
    }
    return true;
...
bool pe32::set_nt_signature(uint32_t &signature) {
    if (!mmap.write(_nt_headers, reinterpret_cast<uint8_t *>(&signature),
            sizeof(uint32_t), offsetof(IMAGE_NT_HEADERS32, Signature))) {
        error_code = mmap.err_code();
        return false;
    }
...
bool pe32::get_file_header(IMAGE_FILE_HEADER &file_header) {
    if (!mmap.read(_nt_headers, reinterpret_cast<uint8_t *>(&file_header),
...
bool pe32::set_file_header(IMAGE_FILE_HEADER &file_header) {
    if (!mmap.write(_nt_headers, reinterpret_cast<uint8_t *>(&file_header),
            sizeof(IMAGE_FILE_HEADER),
            offsetof(IMAGE_NT_HEADERS32, FileHeader))) {
        error_code = mmap.err_code();
        return false;
...
bool pe32::get_optional_header(IMAGE_OPTIONAL_HEADER32 &optional_header) {
    if (!mmap.read(_nt_headers, reinterpret_cast<uint8_t *>(&optional_header),
...
bool pe32::set_optional_header(IMAGE_OPTIONAL_HEADER32 &optional_header) {
    if (!mmap.write(_nt_headers, reinterpret_cast<uint8_t *>(&optional_header),
            sizeof(IMAGE_OPTIONAL_HEADER32),
            offsetof(IMAGE_NT_HEADERS32, OptionalHeader))) {
        error_code = mmap.err_code();
        return false;
...
std::vector<IMAGE_DATA_DIRECTORY> pe32::get_data_table() {
...
bool pe32::set_data_table(std::vector<IMAGE_DATA_DIRECTORY> &data_table) {
    constexpr size_t entries = IMAGE_NUMBEROF_DIRECTORY_ENTRIES;
    if (data_table.size() != entries) {
        return false;
    }
    if (!mmap.write(_nt_headers, reinterpret_cast<uint8_t *>(data_table.data()),
            sizeof(IMAGE_DATA_DIRECTORY) * entries,
            offsetof(IMAGE_NT_HEADERS32, OptionalHeader) +
                offsetof(IMAGE_OPTIONAL_HEADER32, DataDirectory))) {
        error_code = mmap.err_code();
...
std::vector<IMAGE_SECTION_HEADER> pe32::get_section_headers() {
    const size_t count = _nt_headers->FileHeader.NumberOfSections;
    _section_header_list.resize(count);
    if (!mmap.read(_section_header,
            reinterpret_cast<uint8_t *>(_section_header_list.data()),
            static_cast<int64_t>(sizeof(IMAGE_SECTION_HEADER) * count))) {
        error_code = mmap.err_code();
        return {};
    }
...
bool pe32::get_export_table(IMAGE_EXPORT_DIRECTORY &export_table) {
    ...
    if (!mmap.read(_dos_header, reinterpret_cast<uint8_t *>(&export_table),
            sizeof(IMAGE_EXPORT_DIRECTORY), offset)) {
        error_code = mmap.err_code();
        return false;
    }
    return true;
...
bool pe32::set_export_table(IMAGE_EXPORT_DIRECTORY &export_table) {
    if (_data_table_list.empty()) {
        get_data_table();
    }
    if (_data_table_list[DIRECTORY_ENTRY_EXPORT].Size == 0) {
    ...
    if (!mmap.write(_dos_header, reinterpret_cast<uint8_t *>(&export_table),
            sizeof(IMAGE_EXPORT_DIRECTORY), offset)) {
        error_code = mmap.err_code();
        return false;
    }
...
std::vector<std::string> pe32::get_export_func_name_string(
    IMAGE_EXPORT_DIRECTORY *export_table) {
    if (!export_table) {
        return {};
    }
    auto start_name_addr =
        reinterpret_cast<uint32_t *>(reinterpret_cast<uint8_t *>(_dos_header) +
                                     rva_to_foa(export_table->AddressOfNames));
    std::vector<std::string> result;
    for (int32_t i = 0; i < export_table->NumberOfNames; i++) {
        auto name_addr =
            reinterpret_cast<char *>(reinterpret_cast<uint8_t *>(_dos_header)) +
            rva_to_foa(start_name_addr[i]);
        result.emplace_back(name_addr);
    }
    return result;
}

std::vector<uint32_t> pe32::get_export_func_name(
    IMAGE_EXPORT_DIRECTORY *export_table) {
    if (!export_table) {
        return {};
    }
    std::vector<uint32_t> result(export_table->NumberOfNames, 0);
    if (!mmap.read(_dos_header, reinterpret_cast<uint8_t *>(result.data()),
            static_cast<int64_t>(
                sizeof(uint32_t) * export_table->NumberOfNames),
            rva_to_foa(export_table->AddressOfNames))) {
        error_code = mmap.err_code();
        return {};
    }
...
bool pe32::set_export_func_name(IMAGE_EXPORT_DIRECTORY *export_table,
...
std::vector<uint32_t> pe32::get_export_func_addr(
    IMAGE_EXPORT_DIRECTORY *export_table) {
    if (!export_table) {
        return {};
    }
    std::vector<uint32_t> result(export_table->NumberOfFunctions, 0);
    if (!mmap.read(_dos_header, reinterpret_cast<uint8_t *>(result.data()),
            static_cast<int64_t>(
                sizeof(uint32_t) * export_table->NumberOfFunctions),
...
bool pe32::set_export_func_addr(IMAGE_EXPORT_DIRECTORY *export_table,
    std::vector<uint32_t>                              &func_addrs) {
    if (!export_table || func_addrs.size() != export_table->NumberOfFunctions) {
        return false;
    }
    if (!mmap.write(_dos_header, reinterpret_cast<uint8_t *>(func_addrs.data()),
            static_cast<int64_t>(
                sizeof(uint32_t) * export_table->NumberOfFunctions),
            rva_to_foa(export_table->AddressOfFunctions))) {
        error_code = mmap.err_code();
...
std::vector<uint16_t> pe32::get_export_func_ordinal(
    IMAGE_EXPORT_DIRECTORY *export_table) {
    if (!export_table) {
        return {};
    }
...
bool pe32::set_export_func_ordinal(IMAGE_EXPORT_DIRECTORY *export_table,
...
std::vector<IMAGE_IMPORT_DESCRIPTOR> pe32::get_import_table() {
    if (_data_table_list.empty()) {
        get_data_table();
    }
    ...
    size_t count = 0;
    while (import_table->OriginalFirstThunk || import_table->TimeDateStamp ||
           import_table->ForwarderChain || import_table->Name ||
    ...
    if (!mmap.read(_dos_header, reinterpret_cast<uint8_t *>(result.data()),
            static_cast<int64_t>(sizeof(IMAGE_IMPORT_DESCRIPTOR) * count),
            offset)) {
        error_code = mmap.err_code();
        return {};
    }
...
std::vector<std::string> pe32::get_import_table_dll_name_strings(
    std::vector<IMAGE_IMPORT_DESCRIPTOR> &import_table) {
    if (import_table.empty()) {
        return {};
    }
...
std::vector<uint32_t> pe32::get_import_table_dll_name(
    std::vector<IMAGE_IMPORT_DESCRIPTOR> &import_table) {
    if (import_table.empty()) {
        return {};
    }
...
bool pe32::set_import_table_dll_name(
    std::vector<IMAGE_IMPORT_DESCRIPTOR> &import_table,
...
std::vector<uint32_t> pe32::get_import_table_first_thunk(
    std::vector<IMAGE_IMPORT_DESCRIPTOR> &import_table) {
    if (import_table.empty()) {
        return {};
    }
...
bool pe32::set_import_table_first_thunk(
    std::vector<IMAGE_IMPORT_DESCRIPTOR> &import_table,
...
std::vector<IMAGE_THUNK_DATA32> pe32::get_import_table_thunk_data(
    ...
    std::vector<IMAGE_THUNK_DATA32> result(count, {});
    if (!mmap.read(_dos_header, reinterpret_cast<uint8_t *>(result.data()),
            static_cast<int64_t>(sizeof(IMAGE_THUNK_DATA32) * count), offset)) {
        error_code = mmap.err_code();
        return {};
    }
    return result;
...
bool pe32::set_import_table_thunk_data(uint32_t &first_thunk,
    ...
    if (!mmap.write(_dos_header,
            ...
            static_cast<int64_t>(
                sizeof(IMAGE_THUNK_DATA32) * thunk_datas.size()),
            offset)) {
        error_code = mmap.err_code();
        return false;
    }
...
std::vector<std::string> pe32::get_import_table_func_name_strings(
    std::vector<IMAGE_THUNK_DATA32> &thunk_datas) {
    if (thunk_datas.empty()) {
        return {};
    }
...
std::vector<pe32::ImportTableFuncName> pe32::get_import_table_func_name(
    std::vector<IMAGE_THUNK_DATA32> &thunk_datas) {
    if (thunk_datas.empty()) {
        return {};
    }
...
bool pe32::set_import_table_func_name(
    std::vector<IMAGE_THUNK_DATA32>  &thunk_datas,
    ...
    for (int32_t i = 0; i < thunk_datas.size(); i++) {
        if (!(thunk_datas[i].u1.AddressOfData & IMAGE_ORDINAL_FLAG32)) {
            ...
            auto len =
                strlen(reinterpret_cast<char *>(&(func_name_addr->Name)));
...
std::vector<std::pair<uint64_t, uint32_t>> pe32::get_import_table_func_ordinal(
    std::vector<IMAGE_THUNK_DATA32> &thunk_datas) {
    if (thunk_datas.empty()) {
        return {};
    }
    ...
    for (const auto &entry : thunk_datas) {
        if (entry.u1.Ordinal & IMAGE_ORDINAL_FLAG32) {
            result.emplace_back(entry.u1.Ordinal,
                static_cast<uint32_t>(entry.u1.Ordinal & 0xFFFF));
        } else {
...
bool pe32::set_import_table_func_ordinal(uint32_t &first_thunk,
    std::vector<IMAGE_THUNK_DATA32>               &thunk_datas,
    ...
    for (int32_t i = 0; i < thunk_datas.size(); i++) {
        if (thunk_datas[i].u1.Ordinal & IMAGE_ORDINAL_FLAG32) {
...
pe32::get_import_table_forwarder_chain(
    std::vector<IMAGE_IMPORT_DESCRIPTOR> &import_table) {
    if (import_table.empty()) {
        return {};
    }
    ...
    for (const auto &entry : import_table) {
        result.emplace_back(entry.ForwarderChain, entry.OriginalFirstThunk);
    }
    return result;
}

bool pe32::set_import_table_forwarder_chain(
    std::vector<IMAGE_IMPORT_DESCRIPTOR>       &import_table,
...
std::vector<std::string> pe32::get_import_table_forwarder_string(
    std::vector<std::pair<uint32_t, uint32_t>> &forwarder_chain) {
...
std::vector<uint64_t> pe32::get_import_table_forwarder_string_addr(
    std::vector<std::pair<uint32_t, uint32_t>> &forwarder_chain) {
    if (forwarder_chain.empty()) {
        return {};
    }
...
bool pe32::set_import_table_forwarder_string_addr(
    std::vector<std::pair<uint32_t, uint32_t>> &forwarder_chain,
...
std::vector<pe32::RelocationTable> pe32::get_relocation_table() {
    if (_data_table_list.empty()) {
        get_data_table();
    }
    ...
    while (relocation_table->VirtualAddress) {
        ...
        auto item_count =
            (relocation_table->SizeOfBlock - sizeof(IMAGE_BASE_RELOCATION)) /
            sizeof(uint16_t);

        relocation.items.resize(item_count);
        memcpy_s(relocation.items.data(), sizeof(uint16_t) * item_count,
            item_addr, sizeof(uint16_t) * item_count);
...
bool pe32::set_relocation_table(
    ...
    for (const auto &entry : relocation_table) {
        auto item_addr = reinterpret_cast<uint16_t *>(
            reinterpret_cast<uint8_t *>(table) + sizeof(IMAGE_BASE_RELOCATION));
        auto item_count = (table->SizeOfBlock - sizeof(IMAGE_BASE_RELOCATION)) /
                          sizeof(uint16_t);
        memcpy_s(item_addr, sizeof(uint16_t) * item_count, entry.items.data(),
        ...
        table                 = reinterpret_cast<IMAGE_BASE_RELOCATION *>(
            reinterpret_cast<uint8_t *>(table) + table->SizeOfBlock);
    }
    return true;
}

std::vector<std::tuple<uint16_t, uint16_t, uint32_t>>
pe32::get_relocation_table_item(RelocationTable &relocation) {
    if (!relocation.virtual_address || !relocation.size_of_block) {
        return {};
    }
    std::vector<std::tuple<uint16_t, uint16_t, uint32_t>> result;
    for (const auto &entry : relocation.items) {
        uint16_t type      = entry >> 12;
        uint32_t real_addr = relocation.virtual_address + (entry & 0x0FFF);
        result.emplace_back(
            entry, type, (IMAGE_REL_BASED_HIGHLOW == type) ? real_addr : 0);
    }
    return result;
}

bool pe32::set_relocation_table_item(RelocationTable      &relocation,
    ...
    for (int32_t i = 0; i < relocation.items.size(); i++) {
        relocation.items[i] =
            ...
            ((std::get<2>(items[i]) - relocation.virtual_address) & 0x0FFF);
    }
    return true;
}

IMAGE_SECTION_HEADER pe32::find_section_header(uint32_t rva) {
    ...
    for (const auto &section : _section_header_list) {
        if (rva >= section.VirtualAddress &&
            ...
            memcpy_s(&result, sizeof(IMAGE_SECTION_HEADER), &section,
...
int64_t pe32::rva_to_foa(uint32_t rva) {
    int64_t result = 0;
    if (_section_header_list.empty()) {
        get_section_headers();
    }
...
uint32_t pe32::foa_to_rva(IMAGE_SECTION_HEADER &section_header, int64_t foa) {
    return foa - section_header.PointerToRawData +
           section_header.VirtualAddress;
}

std::string pe32::datetime(uint32_t timestamp) {
    FILETIME       ft, ftLocal;
    SYSTEMTIME     st;
    ULARGE_INTEGER uli;
    st.wYear         = 1970;
...
uint32_t pe32::err_code() const {
    return error_code;
}

std::string pe32::err_string() const {
    std::string result = helper::convert::err_string(error_code);
    return result;
}

std::wstring pe32::err_wstring() const {
