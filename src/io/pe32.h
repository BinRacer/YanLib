...
public:
    ...
    bool set_import_table_thunk_data(uint32_t &first_thunk,
    ...
    std::vector<std::pair<uint64_t, uint32_t>> get_import_table_func_ordinal(
        std::vector<IMAGE_THUNK_DATA32> &thunk_datas);

    bool set_import_table_func_ordinal(uint32_t    &first_thunk,
        std::vector<IMAGE_THUNK_DATA32>            &thunk_datas,
        std::vector<std::pair<uint64_t, uint32_t>> &func_ordinals);

    // std::pair<ForwarderChain,OriginalFirstThunk>
    std::vector<std::pair<uint32_t, uint32_t>> get_import_table_forwarder_chain(
        std::vector<IMAGE_IMPORT_DESCRIPTOR> &import_table);
    ...
    bool set_relocation_table(std::vector<RelocationTable> &relocation_table);
    ...
    bool set_relocation_table_item(RelocationTable            &relocation,
    ...
    uint32_t foa_to_rva(IMAGE_SECTION_HEADER &section_header, int64_t foa);
