...
namespace YanLib::io {
pe64::pe64(const char *file_name) {
    do {
...
pe64::pe64(const wchar_t *file_name) {
    do {
        mmap_handle = mmap.create(file_name, static_cast<wchar_t *>(nullptr));
        if (!mmap_handle) {
            error_code = mmap.err_code();
            break;
        }
...
bool pe64::get_nt_signature(uint32_t &signature) {
    if (!mmap.read(_nt_headers, reinterpret_cast<uint8_t *>(&signature),
            sizeof(uint32_t), offsetof(IMAGE_NT_HEADERS64, Signature))) {
        error_code = mmap.err_code();
        return false;
    }
    return true;
...
bool pe64::set_nt_signature(uint32_t &signature) {
...
std::vector<uint32_t> pe64::get_export_func_name(
    IMAGE_EXPORT_DIRECTORY *export_table) {
    if (!export_table) {
        return {};
    }
...
std::vector<uint32_t> pe64::get_export_func_addr(
    IMAGE_EXPORT_DIRECTORY *export_table) {
    if (!export_table) {
        return {};
    }
...
std::vector<IMAGE_THUNK_DATA64> pe64::get_import_table_thunk_data(
    ...
    if (!mmap.read(_dos_header, reinterpret_cast<uint8_t *>(result.data()),
            sizeof(IMAGE_THUNK_DATA64) * count, offset)) {
        error_code = mmap.err_code();
        return {};
    }
    return result;
...
bool pe64::set_import_table_thunk_data(uint32_t &first_thunk,
    ...
    if (!mmap.write(_dos_header,
            ...
            sizeof(IMAGE_THUNK_DATA64) * thunk_datas.size(), offset)) {
        error_code = mmap.err_code();
        return false;
    }
    return true;
...
std::vector<std::pair<uint64_t, uint32_t>> pe64::get_import_table_func_ordinal(
    std::vector<IMAGE_THUNK_DATA64> &thunk_datas) {
    if (thunk_datas.empty()) {
        return {};
    }
...
bool pe64::set_import_table_func_ordinal(uint32_t &first_thunk,
    std::vector<IMAGE_THUNK_DATA64>               &thunk_datas,
...
pe64::get_relocation_table_item(RelocationTable &relocation) {
    ...
    for (const auto &entry : relocation.items) {
        ...
        result.emplace_back(
            entry, type, (IMAGE_REL_BASED_DIR64 == type) ? real_addr : 0);
    }
    return result;
}

bool pe64::set_relocation_table_item(RelocationTable      &relocation,
...
IMAGE_SECTION_HEADER pe64::find_section_header(uint32_t rva) {
...
int64_t pe64::rva_to_foa(uint32_t rva) {
    int64_t result = 0;
    if (_section_header_list.empty()) {
        get_section_headers();
    }
...
uint32_t pe64::foa_to_rva(IMAGE_SECTION_HEADER &section_header, int64_t foa) {
