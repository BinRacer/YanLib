...
int32_t udp_server::read(char *buf,
    ...
    if (is_ipv6) {
        ...
        int32_t      number_of_bytes = recvfrom(server_socket, buf, len, 0,
                 reinterpret_cast<sockaddr *>(&client_addr), &client_size);
        if (number_of_bytes == SOCKET_ERROR) {
            error_code = WSAGetLastError();
        } else {
            if (!inet_ntop(AF_INET6, &client_addr.sin6_addr, client_ip.data(),
                    INET6_ADDRSTRLEN)) {
                error_code = WSAGetLastError();
            }
...
int32_t udp_server::write(char *buf,
    ...
    if (is_ipv6) {
        ...
        int32_t number_of_bytes = sendto(server_socket, buf, len, 0,
...
int32_t udp_server::write_string(std::string &str,
    ...
    return write(
        str.data(), static_cast<int32_t>(str.size()), client_ip, client_port);
}
