...
HANDLE mmap::create(const char *file_name,
    ...
    uint32_t                    max_high,
    uint32_t                    max_low) {
    HANDLE file_handle = CreateFileA(file_name, GENERIC_READ | GENERIC_WRITE,
    ...
    HANDLE mmap_handle = CreateFileMappingA(file_handle, sa,
        static_cast<uint32_t>(protect), max_high, max_low, mmap_name);
    if (!mmap_handle) {
        error_code = GetLastError();
        return nullptr;
    }
...
HANDLE mmap::create(const wchar_t *file_name,
    ...
    uint32_t                       max_high,
    uint32_t                       max_low) {
    HANDLE file_handle = CreateFileW(file_name, GENERIC_READ | GENERIC_WRITE,
        FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING,
        FILE_ATTRIBUTE_NORMAL, nullptr);
    if (file_handle == INVALID_HANDLE_VALUE) {
        error_code = GetLastError();
...
HANDLE mmap::open(const char *mmap_name, MemoryAccess access, bool is_inherit) {
    HANDLE mmap_handle = OpenFileMappingA(
        static_cast<uint32_t>(access), is_inherit ? TRUE : FALSE, mmap_name);
    if (!mmap_handle) {
        error_code = GetLastError();
        return nullptr;
    }
...
mmap::open(const wchar_t *mmap_name, MemoryAccess access, bool is_inherit) {
    HANDLE mmap_handle = OpenFileMappingW(
        static_cast<uint32_t>(access), is_inherit ? TRUE : FALSE, mmap_name);
    if (!mmap_handle) {
        error_code = GetLastError();
        return nullptr;
...
void *mmap::mmap_file(HANDLE mmap_handle,
    ...
    uint32_t                 offset_low,
    SIZE_T                   size) {
    void *address = MapViewOfFile(mmap_handle, static_cast<uint32_t>(access),
...
bool mmap::read(void *addr, uint8_t *buf, int64_t size, uint64_t offset) const {
    if (!buf || !addr) {
        return false;
    }
