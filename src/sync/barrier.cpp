...
barrier::barrier(long total_threads, long spin_count) {
    if (InitializeSynchronizationBarrier(
            &synchronization_barrier, total_threads, spin_count)) {
        error_code = 0;
    }
...
bool barrier::enter(BarrierFlag flag) {
    return EnterSynchronizationBarrier(
        &synchronization_barrier, static_cast<uint32_t>(flag));
}
...
bool barrier::wait(uint32_t count,
    ...
    uint32_t ret = WaitForMultipleObjects(
        count, handles, wait_all ? TRUE : FALSE, milli_seconds);
    if (ret == WAIT_FAILED) {
        error_code = GetLastError();
    } else {
        error_code = ret;
