...
bool timer::create(SECURITY_ATTRIBUTES *sa,
    ...
    timer_handle =
        CreateWaitableTimerW(sa, is_manual_reset ? TRUE : FALSE, timer_name);
...
bool timer::open(const wchar_t *timer_name,
    TimerAccess                 access,
    bool                        is_inherit) {
    timer_handle = OpenWaitableTimerW(
        static_cast<uint32_t>(access), is_inherit ? TRUE : FALSE, timer_name);
    error_code = GetLastError();
    return timer_handle != nullptr;
}
