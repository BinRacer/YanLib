...
HANDLE job::open(const char *job_name, JobAccess access, bool is_inherit) {
    HANDLE job_handle = OpenJobObjectA(
        static_cast<uint32_t>(access), is_inherit ? TRUE : FALSE, job_name);
    if (!job_handle) {
        error_code = GetLastError();
        return nullptr;
    }
...
HANDLE job::open(const wchar_t *job_name, JobAccess access, bool is_inherit) {
    HANDLE job_handle = OpenJobObjectW(
        static_cast<uint32_t>(access), is_inherit ? TRUE : FALSE, job_name);
    if (!job_handle) {
        error_code = GetLastError();
        return nullptr;
