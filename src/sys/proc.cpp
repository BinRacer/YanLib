...
NTSTATUS proc::nt_query_info_proc(HANDLE proc_handle,
    ...
    typedef NTSTATUS(CALLBACK * pfn)(HANDLE ProcessHandle,
        PROCESSINFOCLASS ProcessInformationClass, void *ProcessInformation,
        uint32_t ProcessInformationLength, uint32_t *ReturnLength OPTIONAL);
    NTSTATUS status       = -1;
    ...
    do {
        ...
        status = query_info_proc(
            proc_handle, proc_info_class, proc_info, proc_info_len, real_size);
    } while (false);
    if (ntdll_module) {
...
PROCESS_INFORMATION proc::create(const char *app_name,
    ...
    return create(app.data(), cmd.data(), proc_attrs, thread_attrs, is_inherit,
...
PROCESS_INFORMATION proc::create(const wchar_t *app_name,
    ...
    if (!CreateProcessW(app_name, cmdline, proc_attrs, thread_attrs,
            is_inherit ? TRUE : FALSE, static_cast<uint32_t>(create_flag), env,
            curr_dir, &si, &pi)) {
        error_code = GetLastError();
        return {};
    }
...
PROCESS_INFORMATION proc::create_with_suspended(const char *app_name,
    const char                                             *cmdline,
    SECURITY_ATTRIBUTES                                    *proc_attrs,
    SECURITY_ATTRIBUTES                                    *thread_attrs,
    bool                                                    is_inherit,
    ...
    return create_with_suspended(app.data(), cmd.data(), proc_attrs,
        thread_attrs, is_inherit, create_flag, nullptr, curr.data());
}
...
PROCESS_INFORMATION proc::create_as_user(HANDLE token_handle,
    ...
    if (!CreateProcessAsUserW(token_handle, app_name, cmdline, proc_attrs,
            ...
            static_cast<uint32_t>(create_flag), environment, curr_dir, &si,
            &pi)) {
        error_code = GetLastError();
        return {};
    }
...
PROCESS_INFORMATION proc::create_session_zero(const char *app_name,
    const char                                           *cmdline,
    SECURITY_ATTRIBUTES                                  *proc_attrs,
    SECURITY_ATTRIBUTES                                  *thread_attrs,
    bool                                                  is_inherit,
    ...
    std::wstring cmd  = helper::convert::str_to_wstr(cmdline);
    std::wstring curr = helper::convert::str_to_wstr(curr_dir);
    return create_session_zero(app.data(), cmd.data(), proc_attrs, thread_attrs,
...
PROCESS_INFORMATION proc::create_with_logon(const char *username,
    ...
    std::wstring cmd  = helper::convert::str_to_wstr(cmdline);
    std::wstring curr = helper::convert::str_to_wstr(curr_dir);
    return create_with_logon(user.data(), dom.data(), pass.data(), app.data(),
        cmd.data(), logon_flag, create_flag, nullptr, curr.data());
}
...
PROCESS_INFORMATION proc::create_with_logon(const wchar_t *username,
    ...
    if (!CreateProcessWithLogonW(username, domain, password,
            static_cast<uint32_t>(logon_flag), app_name, cmdline,
            static_cast<uint32_t>(create_flag), environment, curr_dir, &si,
            &pi)) {
        error_code = GetLastError();
        return {};
...
PROCESS_INFORMATION proc::create_with_token(HANDLE token_handle,
    ...
    std::wstring cmd  = helper::convert::str_to_wstr(cmdline);
    std::wstring curr = helper::convert::str_to_wstr(curr_dir);
    return create_with_token(token_handle, app.data(), cmd.data(), logon_flag,
        create_flag, nullptr, curr.data());
}
...
bool proc::win_exec(const char *cmdline, WindowFlag show_flag) {
...
bool proc::fake_proc(HANDLE proc_handle,
    ...
    do {
        NTSTATUS status = nt_query_info_proc(
            proc_handle, ProcessBasicInformation, &pbi, sizeof(pbi), &size);
        ...
        if (!ReadProcessMemory(proc_handle, peb->ProcessParameters,
                params.get(), sizeof(RTL_USER_PROCESS_PARAMETERS), &ret_size)) {
            error_code = GetLastError();
            break;
        }
        uint16_t len = (wcslen(cmdline) + 1) * sizeof(wchar_t);
        if (!WriteProcessMemory(proc_handle, params->CommandLine.Buffer,
                cmdline, len, nullptr)) {
            error_code = GetLastError();
            break;
        }
        if (!WriteProcessMemory(proc_handle, &params->CommandLine.Length, &len,
                sizeof(len), nullptr)) {
            error_code = GetLastError();
            break;
        }
        len = (wcslen(app_name) + 1) * sizeof(wchar_t);
        if (!WriteProcessMemory(proc_handle, params->ImagePathName.Buffer,
                app_name, len, nullptr)) {
            error_code = GetLastError();
            break;
        }
        if (!WriteProcessMemory(proc_handle, &params->ImagePathName.Length,
                &len, sizeof(len), nullptr)) {
            error_code = GetLastError();
            break;
        }
        return true;
...
bool proc::wait_child(HANDLE child_proc_handle, uint32_t milli_seconds) {
    if (child_proc_handle) {
        uint32_t ret = WaitForSingleObject(child_proc_handle, milli_seconds);
...
bool proc::resume_child(HANDLE child_thread_handle) {
    if (child_thread_handle) {
        if (ResumeThread(child_thread_handle) == static_cast<uint32_t>(-1)) {
            error_code = GetLastError();
            return false;
        }
        return true;
...
HANDLE proc::pid_to_handle(uint32_t pid, ProcAccess access, bool is_inherit) {
...
uint32_t proc::exit_status(HANDLE proc_handle) {
    ...
    if (!GetExitCodeProcess(
            proc_handle, reinterpret_cast<unsigned long *>(&exit_status))) {
        error_code = GetLastError();
    }
    return exit_status;
}
...
void *proc::malloc(HANDLE proc_handle,
    ...
    void *address = VirtualAllocEx(proc_handle, addr, size,
        static_cast<uint32_t>(type), static_cast<uint32_t>(protect));
    if (!address) {
        error_code = GetLastError();
    }
    return address;
...
bool proc::free(HANDLE proc_handle,
    ...
    if (!VirtualFreeEx(proc_handle, addr, size, static_cast<uint32_t>(type))) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool proc::write(HANDLE proc_handle,
    ...
    if (!WriteProcessMemory(proc_handle, base_addr, buf, size, bytes_written)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
proc::read(HANDLE proc_handle, const void *base_addr, size_t size) {
    ...
    size_t               bytes_read = 0;
    if (!ReadProcessMemory(proc_handle, base_addr, buffer.data(), buffer.size(),
            &bytes_read)) {
        error_code = GetLastError();
        return {};
    }
...
bool proc::write(HANDLE         proc_handle,
    ...
    if (!WriteProcessMemory(
            proc_handle, base_addr, buf.data(), buf.size(), &bytes_write)) {
        error_code = GetLastError();
        return false;
    }
...
std::string proc::get_env_var(const std::string &var_name) {
    ...
    uint32_t    size = GetEnvironmentVariableA(
        var_name.data(), var_value.data(), var_value.size());
    if (size <= 0) {
        error_code = GetLastError();
        return {};
    }
    var_value.resize(size);
    size = GetEnvironmentVariableA(
        var_name.data(), var_value.data(), var_value.size());
    if (size <= 0) {
        error_code = GetLastError();
        return {};
...
std::wstring proc::get_env_var(const std::wstring &var_name) {
    ...
    uint32_t     size = GetEnvironmentVariableW(
        var_name.data(), var_value.data(), var_value.size());
    if (size <= 0) {
        error_code = GetLastError();
        return {};
...
bool proc::set_env_var(const std::string &var_name,
    ...
    if (!SetEnvironmentVariableA(
            var_name.data(), var_name.empty() ? nullptr : var_value.data())) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool proc::get_cmdline(HANDLE proc_handle, std::wstring &cmdline) {
    ...
    do {
        NTSTATUS status = nt_query_info_proc(process_handle,
            ProcessBasicInformation, &pbi, sizeof(pbi), &ret_size);
        if (!NT_SUCCESS(status)) {
            break;
        }
        auto   peb  = std::make_unique<_peb>();
        size_t size = ret_size;
        if (!ReadProcessMemory(process_handle, pbi.PebBaseAddress, peb.get(),
                sizeof(_peb), &size)) {
            error_code = GetLastError();
            break;
        }
        ...
        if (!ReadProcessMemory(process_handle, peb->process_parameters,
                block.get(), sizeof(_process_parameters), &size)) {
            error_code = GetLastError();
            break;
        }
        cmdline.resize(MAX_PATH + 1, L'\0');
        if (!ReadProcessMemory(process_handle, block->cmdline, cmdline.data(),
                cmdline.size(), &size)) {
            error_code = GetLastError();
            break;
        }
...
bool proc::get_owner(HANDLE proc_handle, std::wstring &owner) {
    ...
    security                  security;
    do {
        if (security.enable_privilege(GetCurrentProcess(), L"SeTcbPrivilege")) {
            break;
        }
        ...
        uint32_t size = 0;
        GetTokenInformation(token_handle, TokenUser, nullptr, 0,
        ...
        if (!GetTokenInformation(token_handle, TokenUser, token_user.data(),
                token_user.size(), reinterpret_cast<unsigned long *>(&size))) {
            error_code = GetLastError();
            break;
        }
        SID_NAME_USE snu;
        TCHAR        user[MAX_PATH];
        uint32_t     user_size     = MAX_PATH;
        uint32_t    *user_size_ptr = &user_size;
        TCHAR        domain[MAX_PATH];
        uint32_t     domain_size     = MAX_PATH;
        uint32_t    *domain_size_ptr = &domain_size;
        if (!LookupAccountSidW(nullptr,
                reinterpret_cast<TOKEN_USER *>(token_user.data())->User.Sid,
                user, reinterpret_cast<unsigned long *>(user_size_ptr), domain,
                reinterpret_cast<unsigned long *>(domain_size_ptr), &snu)) {
            error_code = GetLastError();
            break;
        }
        owner.assign(L"\\\\");
        ...
        owner.append(L"\\");
        owner.append(user);
        if (security.disable_privilege(
                GetCurrentProcess(), L"SeTcbPrivilege")) {
            break;
        }
        return true;
    } while (false);
...
bool proc::get_image_name(HANDLE proc_handle,
    ...
    if (!QueryFullProcessImageNameA(process_handle,
            native_name ? PROCESS_NAME_NATIVE : 0, image_name.data(),
            reinterpret_cast<unsigned long *>(&size))) {
        error_code = GetLastError();
        return false;
    }
...
void *proc::image_base() {
    ...
    do {
        ...
        memcpy(&nt_headers64, base + dos_header.e_lfanew, sizeof(nt_headers64));
        ...
        if (nt_headers64.FileHeader.Machine == IMAGE_FILE_MACHINE_AMD64 ||
            ...
            return reinterpret_cast<void *>(
                static_cast<int64_t>(nt_headers32.OptionalHeader.ImageBase));
        }
    } while (false);
    return nullptr;
}
...
void *proc::image_base(HANDLE proc_handle) {
    ...
    PROCESS_BASIC_INFORMATION pbi;
    NTSTATUS                  status =
        nt_query_info_proc(process_handle, ProcessBasicInformation, &pbi,
...
uint32_t proc::handle_count(HANDLE proc_handle) {
    ...
    if (!GetProcessHandleCount(
            proc_handle, reinterpret_cast<unsigned long *>(&count))) {
        error_code = GetLastError();
    }
    return count;
}
...
bool proc::get_info(HANDLE    proc_handle,
    ...
    if (!GetProcessInformation(
            proc_handle, proc_info_class, proc_info, proc_info_size)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool proc::get_mem_info(HANDLE proc_handle,
    ...
    if (!GetProcessMemoryInfo(proc_handle, mem_counters, mem_counters_size)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool proc::get_module_info(HANDLE proc_handle,
    ...
    if (!GetModuleInformation(
            proc_handle, module_handle, mod_info, mod_info_size)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool proc::logical_processor_info(SYSTEM_LOGICAL_PROCESSOR_INFORMATION *buf,
    ...
    if (!GetLogicalProcessorInformation(
            buf, reinterpret_cast<unsigned long *>(real_size))) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool proc::get_affinity_mask(HANDLE proc_handle,
    ...
    if (!GetProcessAffinityMask(
            proc_handle, proc_affinity_mask, system_affinity_mask)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool proc::set_affinity_mask(HANDLE proc_handle, DWORD_PTR proc_affinity_mask) {
...
bool proc::get_mitigation_policy(HANDLE proc_handle,
    ...
    if (!GetProcessMitigationPolicy(
            proc_handle, mitigation_policy, buffer, len)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool proc::set_priority_boost(HANDLE proc_handle,
    ...
    if (!SetProcessPriorityBoost(
            proc_handle, is_disable_priority_boost ? TRUE : FALSE)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool proc::get_shutdown_params(uint32_t *level, bool *show_retry_dialog) {
    uint32_t flag = 0;
    if (!GetProcessShutdownParameters(reinterpret_cast<unsigned long *>(level),
...
bool proc::time_statistics(HANDLE proc_handle,
    ...
    FILETIME                     *user_time) {
    if (!GetProcessTimes(
            proc_handle, creation_time, exit_time, kernel_time, user_time)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool proc::get_working_set_size(HANDLE proc_handle,
    ...
    if (GetProcessWorkingSetSize(
            proc_handle, min_working_set_size, max_working_set_size)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool proc::get_processor_system_cycle_time(uint16_t group,
    SYSTEM_PROCESSOR_CYCLE_TIME_INFORMATION        *buffer,
    uint32_t                                       *real_size) {
    if (!GetProcessorSystemCycleTime(
            group, buffer, reinterpret_cast<unsigned long *>(real_size))) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool proc::get_affinity_update_mode(HANDLE proc_handle, bool *auto_update) {
    uint32_t flag = 0;
    if (!QueryProcessAffinityUpdateMode(
            proc_handle, reinterpret_cast<unsigned long *>(&flag))) {
        error_code   = GetLastError();
        *auto_update = flag;
        return false;
    }
...
bool proc::set_restriction_exemption(bool is_enable_exemption) {
    if (!SetProcessRestrictionExemption(is_enable_exemption ? TRUE : FALSE)) {
        error_code = GetLastError();
        return false;
    }
    return true;
