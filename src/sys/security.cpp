...
HANDLE security::open_proc_token(HANDLE proc_handle, TokenAccess access) {
    ...
    if (!OpenProcessToken(proc_handle, static_cast<uint32_t>(access), &token)) {
        error_code = GetLastError();
        return nullptr;
    }
    rwlock.write_lock();
...
HANDLE security::copy_token(HANDLE existing_token_handle,
    ...
    HANDLE token_handle =
        existing_token_handle ? existing_token_handle : curr_session_token();
    if (!token_handle) {
        return nullptr;
    }
    HANDLE ret_token = nullptr;
    if (!DuplicateTokenEx(
            token_handle, MAXIMUM_ALLOWED, sa, sil, token_type, &ret_token)) {
        error_code = GetLastError();
        return nullptr;
    }
    rwlock.write_lock();
...
void *security::create_env_block(HANDLE token_handle, bool is_inherit) {
    ...
    if (!CreateEnvironmentBlock(
            &env, token_handle, is_inherit ? TRUE : FALSE)) {
        error_code = GetLastError();
        return nullptr;
    }
    return env;
...
SECURITY_DESCRIPTOR security::create_descriptor(bool is_dacl_present,
    ...
    do {
        ...
        if (!SetSecurityDescriptorDacl(&sd, is_dacl_present ? TRUE : FALSE, acl,
...
bool security::enable_privilege(HANDLE proc_handle, const wchar_t *privilege) {
    helper::autoclean<HANDLE> token_handle(nullptr);
    do {
        if (!OpenProcessToken(
                proc_handle, TOKEN_ADJUST_PRIVILEGES, token_handle)) {
            error_code = GetLastError();
            break;
        }
        TOKEN_PRIVILEGES tp;
        ...
        if (!LookupPrivilegeValueW(
                nullptr, privilege, &tp.Privileges[0].Luid)) {
            error_code = GetLastError();
            break;
        }
        tp.Privileges[0].Attributes = SE_PRIVILEGE_ENABLED;
        if (!AdjustTokenPrivileges(
                token_handle, FALSE, &tp, sizeof(tp), nullptr, nullptr)) {
            error_code = GetLastError();
            break;
        }
...
bool security::disable_privilege(HANDLE proc_handle, const wchar_t *privilege) {
    helper::autoclean<HANDLE> token_handle(nullptr);
    do {
        if (!OpenProcessToken(
                proc_handle, TOKEN_ADJUST_PRIVILEGES, token_handle)) {
...
std::pair<TOKEN_ELEVATION_TYPE, bool> security::check_proc_elevation(
    ...
    do {
        ...
        if (!GetTokenInformation(token_handle, TokenElevationType, &token_type,
        ...
        if (!CreateWellKnownSid(WinBuiltinAdministratorsSid, nullptr,
                &admin_sid, reinterpret_cast<unsigned long *>(&size))) {
            error_code = GetLastError();
            break;
        }
...
IntegrityLevel security::check_proc_integrity_level(HANDLE proc_handle) {
    ...
    do {
        ...
        uint32_t size = 0;
        if (!GetTokenInformation(token_handle, TokenIntegrityLevel, nullptr, 0,
                reinterpret_cast<unsigned long *>(&size))) {
            error_code = GetLastError();
        ...
        if (!GetTokenInformation(token_handle, TokenIntegrityLevel, token_info,
                size, reinterpret_cast<unsigned long *>(&size))) {
            error_code = GetLastError();
            break;
        }
        ...
        if (!GetTokenInformation(token_handle, TokenMandatoryPolicy, &policy,
                size, reinterpret_cast<unsigned long *>(&size))) {
            error_code = GetLastError();
            break;
        }
        ...
        ret = GetSecurityInfo(process_handle, SE_KERNEL_OBJECT,
            LABEL_SECURITY_INFORMATION, nullptr, nullptr, nullptr, &sacl, &sd);
    ...
    return std::make_tuple(
        security_level, token_policy, resource_level, system_policy);
}
