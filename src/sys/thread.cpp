...
HANDLE thread::create(LPTHREAD_START_ROUTINE start_addr,
    ...
    uint32_t tid           = 0;
    HANDLE   thread_handle = CreateThread(sa, stack_size, start_addr, params, 0,
...
HANDLE thread::create_with_suspend(LPTHREAD_START_ROUTINE start_addr,
    ...
    HANDLE   thread_handle = CreateThread(sa, stack_size, start_addr, params,
          CREATE_SUSPENDED, reinterpret_cast<unsigned long *>(&tid));
    if (!thread_handle) {
        error_code = GetLastError();
        return nullptr;
    }
...
HANDLE thread::create_remote(HANDLE proc_handle,
    ...
    HANDLE   thread_handle =
        CreateRemoteThreadEx(proc_handle, sa, stack_size, start_addr, params, 0,
            attr_list, reinterpret_cast<unsigned long *>(&tid));
    if (!thread_handle) {
        error_code = GetLastError();
        return nullptr;
    }
...
HANDLE thread::create_remote_with_suspend(HANDLE proc_handle,
    ...
    SECURITY_ATTRIBUTES                         *sa) {
    uint32_t tid           = 0;
    HANDLE   thread_handle = CreateRemoteThreadEx(proc_handle, sa, stack_size,
          start_addr, params, CREATE_SUSPENDED, attr_list,
          reinterpret_cast<unsigned long *>(&tid));
    if (!thread_handle) {
        error_code = GetLastError();
        return nullptr;
...
HANDLE thread::create_remote_with_stack_reserve(HANDLE proc_handle,
    ...
    uint32_t tid           = 0;
    HANDLE   thread_handle = CreateRemoteThreadEx(proc_handle, sa, stack_size,
          start_addr, params, STACK_SIZE_PARAM_IS_A_RESERVATION, attr_list,
          reinterpret_cast<unsigned long *>(&tid));
    if (!thread_handle) {
        error_code = GetLastError();
        return nullptr;
...
thread::tid_to_handle(uint32_t tid, ThreadAccess access, bool is_inherit) {
    HANDLE thread_handle = OpenThread(
        static_cast<uint32_t>(access), is_inherit ? TRUE : FALSE, tid);
    if (!thread_handle) {
        error_code = GetLastError();
        return nullptr;
    }
...
bool thread::attach_thread_input(uint32_t id_attach,
    ...
    bool                                  is_attach) {
    if (!AttachThreadInput(id_attach, id_attach_to, is_attach ? TRUE : FALSE)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool thread::init_proc_thread_attr_list(LPPROC_THREAD_ATTRIBUTE_LIST attr_list,
    uint32_t                                                         attr_count,
    SIZE_T                                                          *size) {
    if (!InitializeProcThreadAttributeList(attr_list, attr_count, 0, size)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool thread::update_proc_thread_attr(LPPROC_THREAD_ATTRIBUTE_LIST attr_list,
    ...
    if (!UpdateProcThreadAttribute(
            attr_list, 0, attr, value, bytes_size, previous_value, ret_size)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool thread::wait_all(bool is_wait_all, uint32_t milli_seconds) {
    ...
    uint32_t            ret = WaitForMultipleObjects(thread_handles.size(),
                   thread_handles.data(), is_wait_all ? TRUE : FALSE, milli_seconds);
    if (ret >= WAIT_OBJECT_0 &&
...
uint32_t thread::exit_status(HANDLE thread_handle) {
    ...
    if (!GetExitCodeThread(
            thread_handle, reinterpret_cast<unsigned long *>(&exit_code))) {
        error_code = GetLastError();
    }
    return exit_code;
}
...
bool thread::thread_info(HANDLE thread_handle,
    ...
    if (!get_info(
            thread_handle, thread_info_class, thread_info, thread_info_size)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool thread::set_priority_boost(HANDLE thread_handle,
    ...
    if (!SetThreadPriorityBoost(
            thread_handle, is_disable_priority_boost ? TRUE : FALSE)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool thread::time_statistics(HANDLE thread_handle,
    ...
    FILETIME                       *user_time) {
    if (!GetThreadTimes(
            thread_handle, creation_time, exit_time, kernel_time, user_time)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool thread::set_group_affinity(HANDLE thread_handle,
    ...
    if (!SetThreadGroupAffinity(
            thread_handle, group_affinity, previous_group_affinity)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool thread::set_ideal_processor(HANDLE thread_handle,
    ...
    if (!SetThreadIdealProcessorEx(
            thread_handle, ideal_processor, previous_ideal_processor)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool thread::query_idle_processor_cycle_time(uint16_t group,
...
DWORD_PTR thread::set_affinity_mask(HANDLE thread_handle,
    ...
    DWORD_PTR ret = SetThreadAffinityMask(thread_handle, thread_affinity_mask);
    if (!ret) {
        error_code = GetLastError();
    }
    return ret;
