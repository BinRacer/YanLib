...
inline ThreadAccess operator|(ThreadAccess a, ThreadAccess b) {
    return static_cast<ThreadAccess>(static_cast<int>(a) | static_cast<int>(b));
}
#endif
class thread {
private:
...
public:
    ...
    bool wait_all(bool is_wait_all = true, uint32_t milli_seconds = INFINITE);
    ...
    HRESULT get_description(HANDLE thread_handle, wchar_t **thread_description);
    ...
    bool query_idle_processor_cycle_time(uint16_t group,
