...
int32_t clipboard::get_format_name(uint32_t format, std::string &format_name) {
    int32_t result = GetClipboardFormatNameA(
        format, format_name.data(), static_cast<int>(format_name.size()));
    if (!result) {
        error_code = GetLastError();
    }
    return result;
...
int32_t clipboard::get_format_name(uint32_t format, std::wstring &format_name) {
    int32_t result = GetClipboardFormatNameW(
        format, format_name.data(), static_cast<int>(format_name.size()));
    if (!result) {
        error_code = GetLastError();
    }
...
int32_t clipboard::get_priority_format(
    ...
    int32_t result = GetPriorityClipboardFormat(format_priority_list.data(),
        static_cast<int>(format_priority_list.size()));
    if (result == -1) {
        error_code = GetLastError();
    }
    return result;
...
bool clipboard::get_updated_formats(std::vector<uint32_t> &formats,
    ...
    if (!GetUpdatedClipboardFormats(
            formats.data(), formats.size(), real_size)) {
        error_code = GetLastError();
        return false;
    }
    return true;
