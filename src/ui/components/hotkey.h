...
#define MODIFIERSKEY
    ...
    inline ModKey operator|(ModKey a, ModKey b) {
        return static_cast<ModKey>(static_cast<uint8_t>(a) |
...
#define RULEFLAG
    ...
    public:
        ...
        hotkey &operator=(const hotkey &other) = delete;

        hotkey &operator=(hotkey &&other) = delete;

        hotkey() = default;

        ~hotkey() = default;

        HWND create(uintptr_t hotkey_id,
        ...
        std::pair<ModKey, VirtualKey> get_hotkey(HWND hotkey_handle);

        void set_hotkey(HWND hotkey_handle, ModKey mod, VirtualKey vk);

        void set_rules(HWND hotkey_handle,
                       RuleFlag rule,
