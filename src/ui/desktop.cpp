...
HDESK desktop::create(const char *desktop_name,
    ...
    uint32_t                      heap_size) {
    HDESK result = CreateDesktopExA(desktop_name, nullptr, nullptr,
        allow_hook ? DF_ALLOWOTHERACCOUNTHOOK : 0,
        static_cast<uint32_t>(access), sa, heap_size, nullptr);
    if (!result) {
...
HDESK desktop::create(const wchar_t *desktop_name,
    ...
    SECURITY_ATTRIBUTES             *sa,
    uint32_t                         heap_size) {
    HDESK result = CreateDesktopExW(desktop_name, nullptr, nullptr,
        allow_hook ? DF_ALLOWOTHERACCOUNTHOOK : 0,
        static_cast<uint32_t>(access), sa, heap_size, nullptr);
    if (!result) {
        error_code = GetLastError();
...
HDESK desktop::open(const char *desktop_name,
    ...
    DesktopAccess               access) {
    HDESK result =
        OpenDesktopA(desktop_name, allow_hook ? DF_ALLOWOTHERACCOUNTHOOK : 0,
            is_inherit ? TRUE : FALSE, static_cast<uint32_t>(access));
    if (!result) {
        error_code = GetLastError();
        return nullptr;
    }
...
HDESK desktop::open(const wchar_t *desktop_name,
    ...
    DesktopAccess                  access) {
    HDESK result =
        OpenDesktopW(desktop_name, allow_hook ? DF_ALLOWOTHERACCOUNTHOOK : 0,
            is_inherit ? TRUE : FALSE, static_cast<uint32_t>(access));
    if (!result) {
        error_code = GetLastError();
        return nullptr;
...
HDESK desktop::open_input(uint32_t flag,
    bool                           is_inherit,
    DesktopAccess                  access) {
    HDESK result = OpenInputDesktop(
        flag, is_inherit ? TRUE : FALSE, static_cast<uint32_t>(access));
    if (!result) {
        error_code = GetLastError();
        return nullptr;
    }
...
bool desktop::close(HDESK desktop_handle) {
    ...
    const auto it = std::find(
        desktop_handles.begin(), desktop_handles.end(), desktop_handle);
...
bool desktop::enum_windows(HDESK desktop_handle,
    WNDENUMPROC                  fn,
    LPARAM                       lparam) {
...
HWINSTA window_station::create(const char *window_station_name,
    ...
    SECURITY_ATTRIBUTES                   *sa) {
    HWINSTA result = CreateWindowStationA(
        window_station_name, flag, static_cast<uint32_t>(access), sa);
    if (!result) {
        error_code = GetLastError();
        return nullptr;
    }
...
HWINSTA window_station::create(const wchar_t *window_station_name,
    ...
    StationAccess                             access,
    SECURITY_ATTRIBUTES                      *sa) {
    HWINSTA result = CreateWindowStationW(
        window_station_name, flag, static_cast<uint32_t>(access), sa);
    if (!result) {
        error_code = GetLastError();
        return nullptr;
...
bool window_station::close(HWINSTA window_station_handle) {
    ...
    rwlock.write_lock();
    const auto it = std::find(window_station_handles.begin(),
...
bool window_station::set_process_window_station(HWINSTA window_station_handle) {
...
bool window_station::get_user_object_info(HANDLE desktop_or_station_handle,
    ...
    uint32_t                                     size,
    uint32_t                                    *real_size) {
    if (!GetUserObjectInformationW(desktop_or_station_handle, index, buf, size,
...
bool window_station::set_user_object_info(HANDLE desktop_or_station_handle,
    ...
    void                                        *buf,
    uint32_t                                     size) {
    if (!SetUserObjectInformationW(
            desktop_or_station_handle, index, buf, size)) {
        error_code = GetLastError();
        return false;
    }
    return true;
