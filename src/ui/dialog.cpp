...
INT_PTR dialog::create(HINSTANCE instance_handle,
    ...
    INT_PTR result = DialogBoxParamA(
        instance_handle, template_name, parent_window_handle, dialog_func, 0L);
    if (result == -1) {
...
INT_PTR dialog::create(HINSTANCE instance_handle,
    ...
    HWND                         parent_window_handle,
    DLGPROC                      dialog_func) {
    INT_PTR result = DialogBoxIndirectParamW(instance_handle, dialog_template,
...
HWND dialog::create_modeless(HINSTANCE instance_handle,
    ...
    HWND                               parent_window_handle,
    DLGPROC                            dialog_func) {
    HWND result = CreateDialogIndirectParamW(instance_handle, dialog_template,
        parent_window_handle, dialog_func, 0L);
    if (!result) {
        error_code = GetLastError();
        return nullptr;
...
bool dialog::set_button_state(HWND dialog_handle,
    ...
    ButtonState                    state) {
    if (!CheckDlgButton(
            dialog_handle, button_id, static_cast<uint32_t>(state))) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool dialog::set_radio_state(HWND dialog_handle,
    ...
    if (!CheckRadioButton(
            dialog_handle, first_button_id, last_button_id, check_button_id)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
int32_t dialog::fill_listbox(HWND dialog_handle,
    ...
    int32_t                       static_id,
    FileType                      file_type) {
    int32_t result = DlgDirListA(dialog_handle, path_spec, listbox_id,
...
int32_t dialog::fill_listbox(HWND dialog_handle,
    ...
    int32_t                       static_id,
    FileType                      file_type) {
    int32_t result = DlgDirListW(dialog_handle, path_spec, listbox_id,
        static_id, static_cast<uint32_t>(file_type));
    if (!result) {
        error_code = GetLastError();
    }
...
int32_t dialog::fill_combobox(HWND dialog_handle,
    ...
    int32_t                        static_id,
    FileType                       file_type) {
    int32_t result = DlgDirListComboBoxA(dialog_handle, path_spec, combobox_id,
        static_id, static_cast<uint32_t>(file_type));
    if (!result) {
        error_code = GetLastError();
    }
...
int32_t dialog::fill_combobox(HWND dialog_handle,
    ...
    int32_t                        static_id,
    FileType                       file_type) {
    int32_t result = DlgDirListComboBoxW(dialog_handle, path_spec, combobox_id,
        static_id, static_cast<uint32_t>(file_type));
    if (!result) {
        error_code = GetLastError();
    }
...
bool dialog::get_item_int(HWND dialog_handle,
    ...
    *value                = GetDlgItemInt(
        dialog_handle, item_id, &is_translated, is_signed ? TRUE : FALSE);
    if (!is_translated) {
        error_code = GetLastError();
        return false;
    }
...
bool dialog::set_item_int(HWND dialog_handle,
    ...
    bool                       is_signed) {
    if (!SetDlgItemInt(
            dialog_handle, item_id, value, is_signed ? TRUE : FALSE)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
uint32_t
dialog::get_item_text(HWND dialog_handle, int32_t item_id, std::string &text) {
    uint32_t result = GetDlgItemTextA(
        dialog_handle, item_id, text.data(), static_cast<int>(text.size()));
    if (!result) {
        error_code = GetLastError();
    }
    return result;
...
uint32_t
dialog::get_item_text(HWND dialog_handle, int32_t item_id, std::wstring &text) {
...
HWND dialog::next_group_item(HWND dialog_handle,
    ...
    HWND result = GetNextDlgGroupItem(
        dialog_handle, item_handle, search_upward ? TRUE : FALSE);
    if (!result) {
        error_code = GetLastError();
    }
    return result;
...
ButtonState dialog::is_button_checked(HWND dialog_handle, int32_t button_id) {
...
bool dialog::set_item_dpi_behavior(HWND dialog_handle,
    ...
    if (!SetDialogControlDpiChangeBehavior(dialog_handle,
...
bool dialog::set_dpi_behavior(HWND dialog_handle,
    ...
    if (!SetDialogDpiChangeBehavior(dialog_handle,
