...
HBITMAP bitmap::create_di_bitmap(HDC dc_handle,
    ...
    return CreateDIBitmap(
        dc_handle, bitmap_info_header, flag, bits.data(), bitmap_info, usage);
}

std::pair<HBITMAP, uint32_t> bitmap::create_dib_section(HDC dc_handle,
    const BITMAPINFO                                       *bitmap_info,
    uint32_t                                                usage,
    void                                                  **bits,
    ...
    HBITMAP result = CreateDIBSection(
        dc_handle, bitmap_info, usage, bits, section_handle, offset);
...
std::pair<bool, uint32_t> bitmap::bit_blt(HDC dc_handle_dst,
    int32_t                                   x,
    int32_t                                   y,
    int32_t                                   width,
    int32_t                                   height,
    ...
    uint32_t error_code = 0;
    if (!BitBlt(
            dc_handle_dst, x, y, width, height, dc_handle_src, x1, y1, rop)) {
        error_code = GetLastError();
...
bool bitmap::mask_blt(HDC dc_handle_dst,
    ...
    int32_t               y_mask,
    uint32_t              rop) {
    return MaskBlt(dc_handle_dst, x_dst, y_dst, width, height, dc_handle_src,
        x_src, y_src, bitmap_mask_handle, x_mask, y_mask, rop);
}
...
bool bitmap::stretch_blt(HDC dc_handle_dst,
    ...
    return StretchBlt(dc_handle_dst, x_dst, y_dst, width_dst, height_dst,
        dc_handle_src, x_src, y_src, width_src, height_src, rop);
}
...
bool bitmap::pat_blt(HDC dc_handle,
    ...
    int32_t              height,
    uint32_t             rop) {
    return PatBlt(dc_handle, x, y, width, height, rop);
}
...
bool bitmap::transparent_blt(HDC dc_handle_dst,
    ...
    return TransparentBlt(dc_handle_dst, x_dst, y_dst, width_dst, height_dst,
        dc_handle_src, x_src, y_src, width_src, height_src, transparent);
}
...
bool bitmap::gradient_fill(HDC      dc_handle,
    ...
    uint32_t                        mode) {
    return GradientFill(dc_handle, vertex.data(), vertex.size(), mesh.data(),
        mesh.size(), mode);
}
...
bool bitmap::ext_flood_fill(HDC dc_handle,
    ...
    COLORREF                    color,
    uint32_t                    type) {
    return ExtFloodFill(dc_handle, x, y, color, type);
}
...
int32_t bitmap::stretch_di_bits(HDC dc_handle,
    ...
    uint32_t                        usage,
    uint32_t                        rop) {
    return StretchDIBits(dc_handle, x_dst, y_dst, width_dst, height_dst, x_src,
        y_src, width_src, height_src, bits.data(), bitmap_info, usage, rop);
}
...
bool bitmap::alpha_blend(HDC dc_handle_dst,
    ...
    return AlphaBlend(dc_handle_dst, x_dst, y_dst, width_dst, height_dst,
        dc_handle_src, x_src, y_src, width_src, height_src, blend_function);
}
...
int32_t bitmap::set_di_bits(HDC dc_handle,
    ...
    return SetDIBits(dc_handle, bitmap_handle, start_line, line_num,
        bits.data(), bitmap_info, color_use);
}
...
int32_t bitmap::set_di_bits_to_device(HDC dc_handle,
    ...
    return SetDIBitsToDevice(dc_handle, x_dst, y_dst, width, height, x_src,
        y_src, start_scan, line_num, bits.data(), bitmap_info, color_use);
}
} // namespace YanLib::ui::gdi
