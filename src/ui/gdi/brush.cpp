...
bool brush::set_brush_org(HDC dc_handle, int32_t x, int32_t y, POINT *point) {
...
std::pair<bool, uint32_t> brush::set_sys_colors(const std::vector<int32_t> &ele,
    const std::vector<COLORREF> &rgb) {
    uint32_t error_code = 0;
    if (!SetSysColors(static_cast<int>(ele.size()), ele.data(), rgb.data())) {
