...
bool coordinate::device_point_to_logic_point(HDC dc_handle,
    ...
    return DPtoLP(dc_handle, point.data(), static_cast<int>(point.size()));
}

bool coordinate::logic_point_to_device_point(HDC dc_handle,
    std::vector<POINT>                          &point) {
    return LPtoDP(dc_handle, point.data(), static_cast<int>(point.size()));
}
...
bool coordinate::set_window(HDC dc_handle, int32_t x, int32_t y, SIZE *size) {
...
bool coordinate::set_viewport(HDC dc_handle, int32_t x, int32_t y, SIZE *size) {
...
bool coordinate::scale_window(HDC dc_handle,
    ...
    int32_t                       y_div_factor,
    SIZE                         *size) {
    return ScaleWindowExtEx(dc_handle, x_mul_factor, x_div_factor, y_mul_factor,
        y_div_factor, size);
}
