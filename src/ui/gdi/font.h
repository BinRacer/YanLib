...
public:
    ...
    static HFONT create_font_indirect(const ENUMLOGFONTEXDVA *enum_log_font);

    static HFONT create_font_indirect(const ENUMLOGFONTEXDVW *enum_log_font);

    static HANDLE
    add_font_mem_resource(void *file_view, uint32_t size, uint32_t *num_fonts);
    ...
    static uint32_t get_font_unicode_ranges(HDC dc_handle, GLYPHSET *glyph_set);
};
} // namespace YanLib::ui::gdi
#endif // FONT_H
