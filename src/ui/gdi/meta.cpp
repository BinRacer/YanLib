...
namespace YanLib::ui::gdi {
    ...
    UINT meta::get_enhance_meta_file_header(HENHMETAFILE enh_meta_file_handle,
                                            UINT size,
                                            ENHMETAHEADER *buf) {
        return GetEnhMetaFileHeader(enh_meta_file_handle, size, buf);
    }

    UINT meta::get_enhance_meta_file_description(HENHMETAFILE enh_meta_file_handle,
                                                 UINT cch_size,
                                                 wchar_t *description) {
    ...
    UINT meta::get_enhance_meta_file_palette_entries(
        HENHMETAFILE enh_meta_file_handle,
        UINT num,
    ...
    UINT meta::get_win_meta_file_bits(HENHMETAFILE enh_meta_file_handle,
                                      UINT size,
                                      uint8_t *buf,
                                      INT map_mode,
                                      HDC dc_handle) {
    ...
    UINT meta::get_enhance_meta_file_bits(HENHMETAFILE enh_meta_file_handle,
                                          UINT size,
                                          uint8_t *buf) {
        return GetEnhMetaFileBits(enh_meta_file_handle, size, buf);
    }

    HENHMETAFILE meta::set_enhance_meta_file_bits(UINT size,
                                                  const uint8_t *buf) {
        return SetEnhMetaFileBits(size, buf);
    }

    bool meta::gdi_comment(HDC dc_handle, UINT size, const uint8_t *buf) {
