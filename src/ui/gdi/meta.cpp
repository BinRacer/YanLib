...
bool meta::enum_enhance_meta_file(HDC dc_handle,
    ...
    return EnumEnhMetaFile(
        dc_handle, enh_meta_file_handle, enh_meta_file_proc, param, rect);
}
...
bool meta::play_enhance_meta_file_record(HDC dc_handle,
    ...
    return PlayEnhMetaFileRecord(
        dc_handle, handles.data(), enh_meta_record, handles.size());
}
...
uint32_t meta::get_enhance_meta_file_description(
    HENHMETAFILE enh_meta_file_handle,
    std::string &description) {
    return GetEnhMetaFileDescriptionA(
        enh_meta_file_handle, description.size(), description.data());
}
...
uint32_t meta::get_enhance_meta_file_palette_entries(
    ...
    return GetEnhMetaFilePaletteEntries(
        enh_meta_file_handle, palette_entry.size(), palette_entry.data());
}
...
uint32_t meta::get_win_meta_file_bits(HENHMETAFILE enh_meta_file_handle,
    ...
    return GetWinMetaFileBits(
        enh_meta_file_handle, size, buf, map_mode, dc_handle);
}

HENHMETAFILE meta::set_win_meta_file_bits(uint32_t size,
    const uint8_t                                 *buf,
    HDC                                            dc_handle,
