...
uint32_t palette::get_palette_entries(HPALETTE palette_handle,
    ...
    return GetPaletteEntries(
        palette_handle, index, palette_entry.size(), palette_entry.data());
}
...
uint32_t palette::get_system_palette_entries(HDC dc_handle,
    ...
    return GetSystemPaletteEntries(
        dc_handle, index, palette_entry.size(), palette_entry.data());
}
...
HPALETTE palette::select_palette(HDC dc_handle,
    ...
    return SelectPalette(
        dc_handle, palette_handle, force_background ? TRUE : FALSE);
}
...
bool palette::resize_palette(HPALETTE palette_handle, uint32_t palette_num) {
