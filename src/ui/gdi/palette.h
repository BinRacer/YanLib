...
namespace YanLib::ui::gdi {
    ...
    public:
        ...
        static uint32_t
        get_palette_entries(HPALETTE palette_handle,
                            uint32_t index,
                            std::vector<PALETTEENTRY> &palette_entry);

        static uint32_t
        set_palette_entries(HPALETTE palette_handle,
                            uint32_t index,
                            const std::vector<PALETTEENTRY> &palette_entry);

        static uint32_t
        get_system_palette_entries(HDC dc_handle,
                                   uint32_t index,
                                   std::vector<PALETTEENTRY> &palette_entry);
        ...
        static bool resize_palette(HPALETTE palette_handle,
