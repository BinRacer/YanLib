...
HICON icon::create(HINSTANCE instance_handle,
    ...
    HICON result = CreateIcon(
        instance_handle, width, height, planes, bits_pixel, and_bits, xor_bits);
    if (!result) {
...
HICON icon::create(uint8_t *res_buf,
    ...
    bool                    is_icon,
    uint32_t                version) {
    HICON result = CreateIconFromResource(
        res_buf, res_size, is_icon ? TRUE : FALSE, version);
    if (!result) {
        error_code = GetLastError();
        return nullptr;
    }
...
HICON icon::create(uint8_t *res_buf,
    ...
    HICON result = CreateIconFromResourceEx(res_buf, res_size,
        is_icon ? TRUE : FALSE, version, desired_width, desired_height,
...
bool icon::draw(HDC dc_handle,
    ...
    DrawIconFlag    flag) {
    if (!DrawIconEx(dc_handle, x_left, y_top, icon_handle, cx_width, cy_width,
...
int32_t icon::lookup_icon_id(uint8_t *res_buf,
    ...
    int32_t                           desired_height,
    IconFlag                          flag) {
    int32_t result = LookupIconIdFromDirectoryEx(res_buf, is_icon,
        desired_width, desired_height, static_cast<uint32_t>(flag));
    if (!result) {
        error_code = GetLastError();
    }
    return result;
...
uint32_t icon::extract(const char *file_name,
    ...
    uint32_t                      *icon_id,
    IconFlag                       flag) {
    return PrivateExtractIconsA(file_name, icon_index, icon_width, icon_height,
        icon_handle.data(), icon_id, icon_handle.size(),
        static_cast<uint32_t>(flag));
}
...
uint32_t icon::extract(const wchar_t *file_name,
    ...
    uint32_t                         *icon_id,
    IconFlag                          flag) {
    return PrivateExtractIconsW(file_name, icon_index, icon_width, icon_height,
        icon_handle.data(), icon_id, icon_handle.size(),
        static_cast<uint32_t>(flag));
}
