...
HBITMAP image::load_bitmap(HINSTANCE instance_handle,
    ...
    int32_t                          cy,
    ImageFlag                        flag) {
    auto bitmap = static_cast<HBITMAP>(LoadImageA(instance_handle, image_name,
        IMAGE_BITMAP, cx, cy, static_cast<uint32_t>(flag)));
    if (!bitmap) {
...
HBITMAP image::load_bitmap(HINSTANCE instance_handle,
    ...
    int32_t                          cy,
    ImageFlag                        flag) {
    auto bitmap = static_cast<HBITMAP>(LoadImageW(instance_handle, image_name,
        IMAGE_BITMAP, cx, cy, static_cast<uint32_t>(flag)));
    if (!bitmap) {
        error_code = GetLastError();
        return nullptr;
...
HICON image::load_icon(HINSTANCE instance_handle,
    ...
    auto icon = static_cast<HICON>(LoadImageA(instance_handle, image_name,
        IMAGE_ICON, cx, cy, static_cast<uint32_t>(flag)));
    if (!icon) {
        error_code = GetLastError();
        return nullptr;
    }
...
HCURSOR image::load_cursor(HINSTANCE instance_handle,
    ...
    int32_t                          cy,
    ImageFlag                        flag) {
    auto cursor = static_cast<HCURSOR>(LoadImageA(instance_handle, image_name,
        IMAGE_CURSOR, cx, cy, static_cast<uint32_t>(flag)));
    if (!cursor) {
        error_code = GetLastError();
        return nullptr;
    }
...
HCURSOR image::load_cursor(HINSTANCE instance_handle,
    ...
    int32_t                          cy,
    ImageFlag                        flag) {
    auto cursor = static_cast<HCURSOR>(LoadImageW(instance_handle, image_name,
        IMAGE_CURSOR, cx, cy, static_cast<uint32_t>(flag)));
    if (!cursor) {
        error_code = GetLastError();
        return nullptr;
...
HBITMAP image::copy_bitmap(HBITMAP bitmap_handle,
    ...
    int32_t                        cy,
    ImageFlag                      flag) {
    auto bitmap = static_cast<HBITMAP>(CopyImage(
        bitmap_handle, IMAGE_BITMAP, cx, cy, static_cast<uint32_t>(flag)));
    if (!bitmap) {
        error_code = GetLastError();
        return nullptr;
    }
...
HICON image::copy_icon(HICON icon_handle,
    ...
    int32_t                  cy,
    ImageFlag                flag) {
    auto icon = static_cast<HICON>(CopyImage(
        icon_handle, IMAGE_ICON, cx, cy, static_cast<uint32_t>(flag)));
    if (!icon) {
        error_code = GetLastError();
        return nullptr;
    }
...
HCURSOR image::copy_cursor(HCURSOR cursor_handle,
    ...
    int32_t                        cy,
    ImageFlag                      flag) {
    auto cursor = static_cast<HCURSOR>(CopyImage(
        cursor_handle, IMAGE_CURSOR, cx, cy, static_cast<uint32_t>(flag)));
    if (!cursor) {
        error_code = GetLastError();
        return nullptr;
    }
...
bool image::close_bitmap(HBITMAP bitmap_handle) {
    ...
    bitmap_rwlock.write_lock();
    const auto it =
        std::find(bitmap_handles.begin(), bitmap_handles.end(), bitmap_handle);
...
bool image::close_icon(HICON icon_handle) {
    ...
    icon_rwlock.write_lock();
    const auto it =
        std::find(icon_handles.begin(), icon_handles.end(), icon_handle);
...
bool image::close_cursor(HCURSOR cursor_handle) {
    ...
    cursor_rwlock.write_lock();
    const auto it =
        std::find(cursor_handles.begin(), cursor_handles.end(), cursor_handle);
