...
HKL keyboard::load_layout(KeyboardID id, KeyboardLayout layout) {
    HKL result = LoadKeyboardLayoutW(
        KbdIDToWString(id).data(), static_cast<uint32_t>(layout));
    if (!result) {
...
HKL keyboard::activate_layout(HKL layout_handle, KeyboardLayout layout) {
    HKL result =
        ActivateKeyboardLayout(layout_handle, static_cast<uint32_t>(layout));
    if (!result) {
        error_code = GetLastError();
    }
    return result;
...
int32_t keyboard::get_layout_list(std::vector<HKL> &layout_handle) {
    int32_t result = GetKeyboardLayoutList(
        static_cast<int>(layout_handle.size()), layout_handle.data());
    if (!result) {
        error_code = GetLastError();
    }
    return result;
...
int32_t keyboard::get_key_name(long param, std::string &text) {
    int32_t result =
        GetKeyNameTextA(param, text.data(), static_cast<int>(text.size()));
    if (!result) {
        error_code = GetLastError();
    }
    return result;
...
int32_t keyboard::get_key_name(long param, std::wstring &text) {
    int32_t result =
        GetKeyNameTextW(param, text.data(), static_cast<int>(text.size()));
    if (!result) {
        error_code = GetLastError();
    }
    return result;
...
int16_t keyboard::get_key_state(VirtualKey vk) {
    return GetKeyState(static_cast<int>(vk));
}
...
int16_t keyboard::get_async_key_state(VirtualKey vk) {
    return GetAsyncKeyState(static_cast<int>(vk));
}
...
void keyboard::send_event(VirtualKey vk,
    ...
    keybd_event(
        static_cast<uint8_t>(vk), scan, static_cast<uint32_t>(kv), extra_info);
}
...
uint8_t keyboard::virtual_key_to_scan_code(VirtualKey vk,
    ...
    UINT scanCode = MapVirtualKeyW(static_cast<uint32_t>(vk), MAPVK_VK_TO_VSC);
    if (is_extended_key) {
        scanCode |= 0xE000;
    }
...
uint32_t
keyboard::map_virtual_key(uint32_t code, MapType map_type, HKL layout_handle) {
    return MapVirtualKeyExW(
        code, static_cast<uint32_t>(map_type), layout_handle);
}
...
int32_t keyboard::to_ascii(VirtualKey vk,
    ...
    uint16_t ch     = 0;
    int32_t  result = ToAscii(static_cast<uint32_t>(vk), scan_code, key_state,
         &ch, menu_active ? 1 : 0);
    if (result == 1) {
...
int32_t keyboard::to_ascii(VirtualKey vk,
    ...
    uint16_t ch     = 0;
    int32_t  result = ToAsciiEx(static_cast<uint32_t>(vk), scan_code, key_state,
         &ch, menu_active ? 1 : 0, layout_handle);
    if (result == 1) {
        *ch1 = static_cast<uint8_t>(ch & 0xFF);
    } else if (result == 2) {
        *ch1 = static_cast<uint8_t>(ch & 0xFF);
...
int32_t keyboard::to_unicode(VirtualKey vk,
    ...
    return ToUnicode(static_cast<uint32_t>(vk), scan_code, key_state,
        buf.data(), static_cast<int>(buf.size()), 2 | (menu_active ? 1 : 0));
}
...
uint32_t keyboard::get_raw_input_device_info(HANDLE device_handle,
    ...
    void                                           *buf,
    uint32_t                                       *size) {
    uint32_t result = GetRawInputDeviceInfoW(device_handle, command, buf, size);
    error_code      = GetLastError();
    return result;
}
...
uint32_t keyboard::get_raw_input_device_list(
    ...
    uint32_t          *real_num) {
    uint32_t result = GetRawInputDeviceList(
        raw_input_device_list, real_num, sizeof(RAWINPUTDEVICELIST));
    if (result == static_cast<uint32_t>(-1)) {
        error_code = GetLastError();
    }
    return result;
...
uint32_t keyboard::get_registered_raw_input_devices(
    ...
    uint32_t      *real_num) {
    uint32_t result = GetRegisteredRawInputDevices(
        raw_input_devices, real_num, sizeof(RAWINPUTDEVICE));
    if (result == static_cast<uint32_t>(-1)) {
        error_code = GetLastError();
    }
    return result;
...
bool keyboard::register_raw_input_devices(
    ...
    if (!RegisterRawInputDevices(
            raw_input_devices, num, sizeof(RAWINPUTDEVICE))) {
        error_code = GetLastError();
        return false;
    }
    return true;
