...
inline KeyboardLayout operator|(KeyboardLayout a, KeyboardLayout b) {
    return static_cast<KeyboardLayout>(
...
inline VirtualKey operator|(VirtualKey a, VirtualKey b) {
    return static_cast<VirtualKey>(static_cast<int>(a) | static_cast<int>(b));
}
#endif
#ifndef KEYEVENT
#define KEYEVENT
...
inline KeyEvent operator|(KeyEvent a, KeyEvent b) {
    return static_cast<KeyEvent>(static_cast<int>(a) | static_cast<int>(b));
}
#endif
#ifndef MAPTYPE
#define MAPTYPE
...
public:
    ...
    int32_t get_key_name(long param, std::string &text);

    int32_t get_key_name(long param, std::wstring &text);
    ...
    bool
    vk_key_scan(char ch, HKL layout_handle, ModKey6 *mod_key, VirtualKey *vk);
    ...
    uint32_t wait_for_input_idle(HANDLE proc_handle, uint32_t milli_seconds);
    ...
    uint32_t
    get_raw_input_buffer(RAWINPUT *buf, uint32_t *size, uint32_t header_size);
    ...
    uint32_t get_raw_input_device_list(
        RAWINPUTDEVICELIST raw_input_device_list[],
        uint32_t          *real_num);

    uint32_t get_registered_raw_input_devices(
