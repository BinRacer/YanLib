...
HMENU menu::load(const std::vector<MenuItemTemplate> &menu_templates) {
    ...
    uint32_t size =
        ...
        (sizeof(MENUITEMTEMPLATE) + sizeof(wchar_t)) * menu_templates.size();
...
#pragma pack(pop)
    uint32_t size =
        sizeof(MenuTemplateHeader) +
        (sizeof(MenuTemplateItem) + sizeof(wchar_t)) * menu_templates.size();
    for (const auto &menu_template : menu_templates) {
        size += (menu_template.text.size() * sizeof(wchar_t));
    }
    std::vector<uint8_t> buffer(size, '\0');
...
bool menu::append(HMENU menu_handle,
    ...
    MenuFlag            flag) {
    if (!AppendMenuA(
            menu_handle, static_cast<uint32_t>(flag), item_id, item_text)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool menu::remove(HMENU menu_handle, uint32_t pos, bool by_pos) {
    if (!RemoveMenu(menu_handle, pos, by_pos ? MF_BYPOSITION : MF_BYCOMMAND)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool menu::release(HMENU menu_handle, uint32_t pos, bool by_pos) {
    ...
    rwlock.write_lock();
    const auto it =
        std::find(menu_handles.begin(), menu_handles.end(), menu_handle);
    if (it != menu_handles.end()) {
        *it = nullptr;
    }
    rwlock.write_unlock();
    if (!DeleteMenu(menu_handle, pos, by_pos ? MF_BYPOSITION : MF_BYCOMMAND)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
uint32_t menu::get_state(HMENU menu_handle, uint32_t pos, bool by_pos) {
    return GetMenuState(
        menu_handle, pos, by_pos ? MF_BYPOSITION : MF_BYCOMMAND);
}
...
int32_t menu::get_string(HMENU menu_handle,
    ...
    return GetMenuStringA(menu_handle, pos, text.data(),
        static_cast<int>(text.size()), by_pos ? MF_BYPOSITION : MF_BYCOMMAND);
}
...
std::pair<int16_t, int16_t> menu::get_marker_bitmap_size() {
    long    size   = GetMenuCheckMarkDimensions();
...
bool menu::insert_item(HMENU menu_handle,
    ...
    bool                     is_pos) {
    if (!InsertMenuItemA(
            menu_handle, pos, is_pos ? TRUE : FALSE, menu_item_info)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
menu::get_default_item(HMENU menu_handle, bool is_pos, MenuSearch search) {
    uint32_t result = GetMenuDefaultItem(
        menu_handle, is_pos ? TRUE : FALSE, static_cast<uint32_t>(search));
...
bool menu::set_item_bitmaps(HMENU menu_handle,
    ...
    if (!SetMenuItemBitmaps(menu_handle, pos,
            is_pos ? MF_BYPOSITION : MF_BYCOMMAND, unchecked_handle,
            checked_handle)) {
        error_code = GetLastError();
        return false;
    }
...
MenuFlag menu::uncheck_item(HMENU menu_handle, uint32_t pos, MenuFlag flag) {
    return static_cast<MenuFlag>(
        CheckMenuItem(menu_handle, pos, static_cast<uint32_t>(flag)));
}
...
MenuFlag menu::disable_item(HMENU menu_handle, uint32_t pos, MenuFlag flag) {
    return static_cast<MenuFlag>(
        EnableMenuItem(menu_handle, pos, static_cast<uint32_t>(flag)));
}
