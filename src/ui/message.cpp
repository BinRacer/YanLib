...
bool message::post(uint32_t tid, uint32_t msg, WPARAM wparam, LPARAM lparam) {
...
long message::broadcast_system(uint32_t msg,
    WPARAM                              wparam,
    LPARAM                              lparam,
    BroadcastResult                    *info,
    BroadcastFlag                       flag) {
    long result = BroadcastSystemMessageW(static_cast<uint32_t>(flag),
        reinterpret_cast<unsigned long *>(info), msg, wparam, lparam);
    if (!result) {
        error_code = GetLastError();
    }
    return result;
...
long message::broadcast_system(uint32_t msg, /* RegularMessage */
    WPARAM                              wparam,
    LPARAM                              lparam,
    BSMINFO                            *bsm_info,
    BroadcastResult                    *info,
    BroadcastFlag                       flag) {
    long result = BroadcastSystemMessageExW(static_cast<uint32_t>(flag),
        reinterpret_cast<unsigned long *>(info), msg, wparam, lparam, bsm_info);
    if (!result) {
        error_code = GetLastError();
    }
    return result;
