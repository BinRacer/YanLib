...
bool scroll::show(HWND window_handle, ScrollType scroll_type) {
    if (!ShowScrollBar(
            window_handle, static_cast<uint32_t>(scroll_type), TRUE)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool scroll::hide(HWND window_handle, ScrollType scroll_type) {
    if (!ShowScrollBar(
            window_handle, static_cast<uint32_t>(scroll_type), FALSE)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool scroll::get_info(HWND window_handle,
    ...
    ObjectID               object_id) {
    if (!GetScrollBarInfo(
            window_handle, static_cast<int32_t>(object_id), scroll_bar_info)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool scroll::get_info(HWND window_handle,
    ...
    ScrollType             scroll_type) {
    if (!GetScrollInfo(
            window_handle, static_cast<uint32_t>(scroll_type), scroll_info)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
int32_t scroll::set_pos(HWND window_handle,
    ...
    ScrollType               scroll_type,
    bool                     is_redraw) {
    int32_t result = SetScrollPos(window_handle,
        static_cast<uint32_t>(scroll_type), pos, is_redraw ? TRUE : FALSE);
    if (!result) {
        error_code = GetLastError();
    }
    return result;
