...
bool touch::inject_synthetic_pointer_input(
    ...
    if (!InjectSyntheticPointerInput(
            device_handle, pointer_info.data(), pointer_info.size())) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool touch::get_pointer_frame_info_history(uint32_t pointer_id,
    uint32_t                                       *row,
    uint32_t                                       *col,
    POINTER_TOUCH_INFO                              touch_info[]) {
    if (!GetPointerFrameTouchInfoHistory(pointer_id, row, col, touch_info)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool touch::init_injection(uint32_t max_count, PointerFeedback mode) {
    if (!InitializeTouchInjection(
            max_count, static_cast<unsigned long>(mode))) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
LRESULT touch::pack_hit_testing_proximity_evaluation(
    ...
    LRESULT result = PackTouchHitTestingProximityEvaluation(
...
bool touch::evaluate_proximity_to_polygon(
    ...
    if (!EvaluateProximityToPolygon(control_polygon.size(),
            control_polygon.data(), hit_testing_input, proximity_eval)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
bool touch::evaluate_proximity_to_rect(const RECT *control_bounding_box,
    const TOUCH_HIT_TESTING_INPUT                 *hit_testing_input,
    TOUCH_HIT_TESTING_PROXIMITY_EVALUATION        *proximity_eval) {
    if (!EvaluateProximityToRect(
            control_bounding_box, hit_testing_input, proximity_eval)) {
        error_code = GetLastError();
        return false;
    }
    return true;
...
long touch::coord_to_pixel(long coordinate) {
