...
HHOOK window_hook::set(int32_t hook_type,
    HOOKPROC                   hook_proc,
    HINSTANCE                  dll_handle,
    uint32_t                   tid) {
    HHOOK result = SetWindowsHookExW(hook_type, hook_proc, dll_handle, tid);
    if (!result) {
...
LRESULT
window_hook::call_next(HHOOK hook_handle,
    int32_t                  code,
    WPARAM                   wparam,
    LPARAM                   lparam) {
...
HWINEVENTHOOK window_hook::set_event(uint32_t event_min,
    uint32_t                                  event_max,
    HMODULE                                   dll_handle,
    ...
    return SetWinEventHook(
        event_min, event_max, dll_handle, event_proc, pid, tid, flag);
}
...
void window_hook::notify_event(uint32_t event,
    HWND                                window_handle,
    int32_t                             object_id,
    int32_t                             child_id) {
